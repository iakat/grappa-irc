...
export function watchPageConsole(page) {
    if (consoleByPage.has(page))
        return;
    const buffer = [];
    consoleByPage.set(page, buffer);
    page.on("console", (message) => {
        buffer.push(`${message.type()}: ${message.text()}`);
        if (buffer.length > CONSOLE_CAP)
            buffer.shift();
    });
}
...
export async function describeUserTopicTimeout(page, userName, budgetMs) {
    ...
    try {
        ...
        return `${head}, and the page could not be interrogated afterwards: ${err instanceof Error ? err.message : String(err)}`;
    }
