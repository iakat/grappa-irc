...
export async function sendPickedFiles(page) {
...
export async function pickFile(page, file, modalHeading) {
...
export async function uploadViaPicker(page, file, opts) {
    const modal = await pickFile(page, file, EMBEDDED_MODAL_HEADING);
    const [uploadRes] = await Promise.all([
        page.waitForResponse((r) => r.url().includes("/api/uploads") && r.request().method() === "POST", { timeout: opts.postTimeout }),
...
export async function mediaScrollbackRow(page, emoji, marker) {
