...
async function loginWithRetry(identifier, password, attempts = 3) {
    let lastErr;
    ...
    throw new Error(`loginWithRetry: ${identifier} failed after ${attempts} attempts: ${String(lastErr)}`);
}
...
// #1911 — user for the OIDC provider round trip (issue1911-oidc-login).
// Created with NO bind in compose.yaml and PRE-LINKED to the stub OP's
// "oidc1911" persona (sub 1911-linked-sub at issuer https://oidc-test:3443
// — see ../oidc-op/server.mjs PERSONAS). No token stash and no getter: the
// spec's whole point is ARRIVING at a session through the provider, so
// globalSetup logging in on its behalf would defeat it (same posture as
// FRESH_USER above).
export const OIDC1911_USER = "oidc1911";
export const OIDC1911_PASSWORD = "test-password-not-secret";
...
export function getSeededFloodVictim() {
    ...
    if (!token || !subjectJson) {
        throw new Error(`getSeededFloodVictim: ${FLOOD_VICTIM_TOKEN_ENV_VAR}/${FLOOD_VICTIM_SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
...
export function getSeededVjt() {
    const token = process.env[TOKEN_ENV_VAR];
    const subjectJson = process.env[SUBJECT_ENV_VAR];
    if (!token || !subjectJson) {
        throw new Error(`getSeededVjt: ${TOKEN_ENV_VAR}/${SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
        name: VJT_USER,
...
export function getSeededAdmin() {
    const token = process.env[ADMIN_TOKEN_ENV_VAR];
    const subjectJson = process.env[ADMIN_SUBJECT_ENV_VAR];
    if (!token || !subjectJson) {
        throw new Error(`getSeededAdmin: ${ADMIN_TOKEN_ENV_VAR}/${ADMIN_SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
        name: ADMIN_USER,
...
export function getSeededWizUser() {
    const token = process.env[WIZ_TOKEN_ENV_VAR];
    const subjectJson = process.env[WIZ_SUBJECT_ENV_VAR];
    if (!token || !subjectJson) {
        throw new Error(`getSeededWizUser: ${WIZ_TOKEN_ENV_VAR}/${WIZ_SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
        name: WIZ_USER,
...
export function getSeededMute1038User() {
    const token = process.env[MUTE1038_TOKEN_ENV_VAR];
    const subjectJson = process.env[MUTE1038_SUBJECT_ENV_VAR];
    if (!token || !subjectJson) {
        throw new Error(`getSeededMute1038User: ${MUTE1038_TOKEN_ENV_VAR}/${MUTE1038_SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
        name: MUTE1038_USER,
...
export function getSeededAccreteUser() {
    const token = process.env[ACCRETE_TOKEN_ENV_VAR];
    const subjectJson = process.env[ACCRETE_SUBJECT_ENV_VAR];
    if (!token || !subjectJson) {
        throw new Error(`getSeededAccreteUser: ${ACCRETE_TOKEN_ENV_VAR}/${ACCRETE_SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
        name: ACCRETE_USER,
...
export function getSeededI498User() {
    const token = process.env[I498_TOKEN_ENV_VAR];
    const subjectJson = process.env[I498_SUBJECT_ENV_VAR];
    if (!token || !subjectJson) {
        throw new Error(`getSeededI498User: ${I498_TOKEN_ENV_VAR}/${I498_SUBJECT_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
    return {
        name: I498_USER,
...
export function getSeededM9bSessionId() {
    const userId = process.env[M9B_USER_ID_ENV_VAR];
    if (!userId) {
        throw new Error(`getSeededM9bSessionId: ${M9B_USER_ID_ENV_VAR} not set. Did playwright globalSetup run?`);
    }
...
export function getSeededM9bVictim() {
    const userId = process.env[M9B_VICTIM_USER_ID_ENV_VAR];
    const token = process.env[M9B_VICTIM_TOKEN_ENV_VAR];
    if (!userId || !token) {
        throw new Error(`getSeededM9bVictim: ${M9B_VICTIM_USER_ID_ENV_VAR}/${M9B_VICTIM_TOKEN_ENV_VAR} not set. ` +
