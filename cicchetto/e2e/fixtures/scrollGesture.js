...
export async function waitForScrollRest(pane, opts) {
    const { timeoutMs, pollMs } = opts;
    const read = () => pane.scrollTop();
    const rest = await sampleUntilRest(read, await read(), false, timeoutMs, pollMs);
    if (rest.settled === null) {
        throw new Error(`waitForScrollRest: the pane never came to rest within ${timeoutMs}ms ` +
...
async function sampleUntilRest(read, baseline, requireMove, timeoutMs, pollMs) {
...
export async function scrollByGesture(pane, opts) {
    ...
    if (rest.settled !== null)
        return { from, to: rest.settled };
    throw new Error(rest.moved
