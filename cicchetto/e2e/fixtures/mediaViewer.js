...
export function mediaViewer(page) {
...
export async function openMediaViewer(page, link) {
    await link.click();
    return settled(page);
}
...
export async function openMediaViewerInPlace(page, link) {
...
export async function closeMediaViewer(viewer) {
...
export async function uploadImageAndGetLink(page, fileName) {
...
export async function uploadSizedImageAndGetLink(page, fileName, size) {
    return uploadPngAndGetLink(page, fileName, pngOfSize(size.width, size.height));
}
async function uploadPngAndGetLink(page, fileName, buffer) {
    const { slug, url } = await uploadViaPicker(page, { name: fileName, mimeType: "image/png", buffer }, { postTimeout: IMAGE_POST_TIMEOUT_MS });
