...
export async function awaitPrivmsg(source, spec) {
    ...
    const matches = (event) => event.nick === fromNick && event.message.includes(body);
    const handler = (event) => {
        const at = Date.now();
    ...
    timer = setTimeout(() => {
        expired = true;
        timer = setTimeout(() => finish(new Error(failureMessage(spec, seen, triggeredAt, undefined))), LATE_ARRIVAL_GRACE_MS);
    }, timeoutMs);
    return wait;
}
function failureMessage(spec, seen, triggeredAt, lateArrivalAt) {
...
function cause(spec, seen, triggeredAt, lateArrivalAt) {
