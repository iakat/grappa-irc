...
export class IrcPeer {
    ...
    async join(channel) {
        const joined = onceMatching(this.client, "join", (event) => event.nick === this.nick && event.channel === channel, JOIN_TIMEOUT_MS, `join ${channel}`);
    ...
    waitForPrivmsg(fromNick, body, trigger, timeoutMs = PRIVMSG_TIMEOUT_MS) {
    ...
    answerCtcpPing() {
        ...
        this.client.on("raw", (event) => {
            if (done || !event.from_server || event.line[0] !== ":")
                return;
    ...
    // well-formed address.
    async nickservRegister(password, email) {
        const noticeReceived = once(this.client, "notice", NICKSERV_TIMEOUT_MS, `nickserv register notice for ${this.nick}`);
    ...
    async nickservIdentify(password) {
        const noticeReceived = once(this.client, "notice", NICKSERV_TIMEOUT_MS, `nickserv identify notice for ${this.nick}`);
        const umodeRSet = onceMatching(this.client, "mode", (event) => event.target === this.nick && event.raw_modes.includes("+r"), NICKSERV_TIMEOUT_MS, `umode +r on ${this.nick}`);
    ...
    async nickservAuth(code) {
        const noticeReceived = once(this.client, "notice", NICKSERV_TIMEOUT_MS, `nickserv auth notice for ${this.nick}`);
        const umodeRSet = onceMatching(this.client, "mode", (event) => event.target === this.nick && event.raw_modes.includes("+r"), NICKSERV_TIMEOUT_MS, `umode +r on ${this.nick}`);
    ...
    async away(message) {
        const ack = onceMatching(this.client, "raw", (event) => typeof event.line === "string" && / 306 /.test(event.line), AWAY_TIMEOUT_MS, `away ack (306 RPL_NOWAWAY) for ${this.nick}`);
    ...
    whoisAway(nick, timeoutMs = WHOIS_TIMEOUT_MS) {
        return awaitWhoisAway(this.client, { nick, timeoutMs });
    }
    async part(channel, reason) {
        const parted = onceMatching(this.client, "part", (event) => event.nick === this.nick && event.channel === channel, PART_TIMEOUT_MS, `part ${channel}`);
    ...
    async mode(channel, rawModes, extraArg) {
    ...
    // overriding.
    async samode(channel, rawModes, extraArg) {
        const modeEcho = this.channelModeEcho(channel, rawModes, extraArg, "samode");
        this.client.raw(extraArg ? ["SAMODE", channel, rawModes, extraArg] : ["SAMODE", channel, rawModes]);
        await modeEcho;
    }
    ...
    channelModeEcho(channel, rawModes, extraArg, verb) {
        return onceMatching(this.client, "mode", (event) => event.target === channel && event.raw_modes === rawModes, MODE_TIMEOUT_MS, `${verb} ${channel} ${rawModes}${extraArg ? ` ${extraArg}` : ""}`);
    }
    ...
    async umode(rawModes) {
        const umodeEcho = onceMatching(this.client, "mode", (event) => event.target === this.nick && event.raw_modes === rawModes, MODE_TIMEOUT_MS, `umode ${this.nick} ${rawModes}`);
    ...
    async topic(channel, text) {
        const topicSet = onceMatching(this.client, "topic", (event) => event.channel === channel && event.topic === text, TOPIC_TIMEOUT_MS, `topic ${channel}`);
    ...
    async waitForTopic(channel, text) {
        await onceMatching(this.client, "topic", (event) => event.channel === channel && event.topic === text, TOPIC_TIMEOUT_MS, `witness topic ${channel}`);
    }
    ...
    async kick(channel, target, reason) {
        const kicked = onceMatching(this.client, "kick", (event) => event.nick === this.nick && event.kicked === target && event.channel === channel, KICK_TIMEOUT_MS, `kick ${channel} ${target}`);
    ...
    async changeNick(newNick) {
        ...
        const renamed = onceMatching(this.client, "nick", (event) => event.nick === oldNick && event.new_nick === newNick, NICK_TIMEOUT_MS, `nick ${oldNick} → ${newNick}`);
    ...
    // it.
    waitForLine(pattern, label, timeoutMs = 8_000) {
        return new Promise((resolve, reject) => {
            ...
            const handler = (event) => {
                if (!event.from_server || !pattern.test(event.line))
                    return;
                clearTimeout(timer);
                this.client.removeListener("raw", handler);
                resolve(event.line);
...
function once(client, event, timeoutMs, label) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
...
function onceMatching(client, event, predicate, timeoutMs, label) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            client.removeListener(event, handler);
            reject(new Error(`IrcPeer: timeout waiting for ${label} (${timeoutMs}ms)`));
