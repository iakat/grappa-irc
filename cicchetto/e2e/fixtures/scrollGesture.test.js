...
async function failureOf(gesture) {
    try {
        await gesture;
    }
    catch (err) {
...
describe("#1336 — scrollByGesture", () => {
    ...
    it("REJECTS when the wheel never moved the pane", async () => {
        ...
        expect(await failureOf(scrollByGesture(pane, { deltaY: -4000, timeoutMs: 20, pollMs: 1 }))).toContain("never moved");
    });
    it("REJECTS when the pane is still moving at the deadline", async () => {
        ...
        expect(await failureOf(scrollByGesture(pane, { deltaY: -4000, timeoutMs: 20, pollMs: 1 }))).toContain("never settled");
    });
});
...
const MEASURED_SWITCH_WRITES = [7, 1055, 1078];
...
describe("#1336 S2 — waitForScrollRest, and the barrier it replaces", () => {
    ...
    it("does not call a pane written every 20ms 'at rest', however fast it is read", async () => {
        ...
        expect(await failureOf(waitForScrollRest(movingPane, { timeoutMs: 300, pollMs: 50 }))).toContain("never came to rest");
    });
