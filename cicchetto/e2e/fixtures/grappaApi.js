...
export async function mintVisitor(nick, incognito = false) {
    const loginBody = { identifier: nick };
    if (incognito)
        loginBody.incognito = true;
    const response = await fetch(`${GRAPPA_BASE_URL}/auth/login`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!netsRes.ok) {
        throw new Error(`grappaApi.mintVisitor: GET /networks → ${netsRes.status} ${await netsRes.text()}`);
    }
...
export async function loginVisitor(nick, password, networkSlug) {
    const response = await fetch(`${GRAPPA_BASE_URL}/auth/login`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!response.ok) {
        throw new Error(`grappaApi.loginVisitor: ${nick} → ${response.status} ${await response.text()}`);
    }
    const body = (await response.json());
    if (body.subject.kind !== "visitor") {
...
export async function listGalleryThemes(token) {
...
export async function copyTheme(token, id) {
...
export async function publishTheme(token, id) {
...
export async function listBuiltinBackgrounds(token) {
...
export async function createThemeWithBuiltinBackground(token, name, builtinKey, opacity) {
    ...
    if (!res.ok) {
        throw new Error(`grappaApi.createThemeWithBuiltinBackground: ${res.status} ${await res.text()}`);
    }
    return (await res.json());
}
...
export async function setActiveTheme(token, light, dark) {
    ...
    if (!res.ok) {
        throw new Error(`grappaApi.setActiveTheme: ${light}/${dark} → ${res.status} ${await res.text()}`);
    }
}
...
export async function adminDeleteTheme(adminToken, id) {
...
async function getMessagesPage(token, networkSlug, channel, before) {
    const base = `${GRAPPA_BASE_URL}/networks/${encodeURIComponent(networkSlug)}/channels/${encodeURIComponent(channel)}/messages`;
...
export async function fetchScrollbackPage(token, networkSlug, channel) {
...
export async function restoreReadCursorToTail(token, networkSlug, channel) {
    const rows = await getMessagesPage(token, networkSlug, channel, undefined);
    const tail = rows[0];
    if (!tail)
        return;
    const cursorUrl = `${GRAPPA_BASE_URL}/networks/${encodeURIComponent(networkSlug)}/channels/${encodeURIComponent(channel)}/read-cursor`;
    ...
    if (!cursorRes.ok) {
        throw new Error(`restoreReadCursorToTail: POST /read-cursor → ${cursorRes.status} ${await cursorRes.text()}`);
    }
}
...
export async function setReadCursorToId(token, networkSlug, channel, messageId) {
    const url = `${GRAPPA_BASE_URL}/networks/${encodeURIComponent(networkSlug)}/channels/${encodeURIComponent(channel)}/read-cursor/force`;
    ...
    if (!res.ok) {
        throw new Error(`setReadCursorToId: POST /read-cursor/force → ${res.status} ${await res.text()}`);
    }
}
...
export async function fetchAllMessagesAsc(token, networkSlug, channel) {
    const byId = new Map();
    let before;
...
export async function terminateSession(adminToken, sessionId) {
...
export async function findUserIdByName(adminToken, userName) {
...
export async function setAdminFlag(adminToken, userId, isAdmin) {
    ...
    if (!res.ok) {
        throw new Error(`grappaApi.setAdminFlag: is_admin=${isAdmin} on ${userId} → ${res.status} ${await res.text()}`);
    }
}
export async function adminPatchCaps(adminToken, slug, caps) {
...
export async function listSessionLogSessions(adminToken) {
...
export async function adminDeleteUploadBySlug(adminToken, slug) {
    const listRes = await fetch(`${GRAPPA_BASE_URL}/admin/uploads`, {
        headers: { authorization: `Bearer ${adminToken}` },
    });
    if (!listRes.ok) {
        throw new Error(`grappaApi.adminDeleteUploadBySlug: GET /admin/uploads → ${listRes.status} ${await listRes.text()}`);
    }
    ...
    if (!delRes.ok) {
        throw new Error(`grappaApi.adminDeleteUploadBySlug: DELETE ${row.id} → ${delRes.status} ${await delRes.text()}`);
    }
}
...
export async function publicUploadStatus(uploadUrl, method) {
...
export async function adminDeleteVisitor(adminToken, visitorId) {
    ...
    if (!res.ok && res.status !== 404) {
        throw new Error(`grappaApi.adminDeleteVisitor: ${visitorId} → ${res.status} ${await res.text()}`);
    }
}
...
export async function visitorExists(adminToken, visitorId) {
...
export async function reapVisitors(adminToken, ...visitorIds) {
    const errors = [];
...
export async function assertMessagePersisted(opts) {
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            if (!(err instanceof MessagesPageStatusError))
                throw err;
            await sleep(intervalMs);
            continue;
        }
        const matched = messages.find((m) => m.sender === opts.sender &&
    ...
    throw new Error(`assertMessagePersisted: timeout after ${timeoutMs}ms — channel=${opts.channel} sender=${opts.sender} body=${JSON.stringify(opts.body)} kind=${JSON.stringify(opts.kind)}; last seen: ${JSON.stringify(lastSeen)}`);
}
function sleep(ms) {
...
export async function listArchiveTargets(token, networkSlug) {
...
export async function partChannel(token, networkSlug, channelName) {
...
export async function awaitPartEcho(token, networkSlug, channel, nick) {
...
export function muteKey(networkSlug, target) {
...
export async function clearMutedConversations(token) {
    const url = `${GRAPPA_BASE_URL}/me/settings/notification-prefs`;
    const headers = { authorization: `Bearer ${token}` };
    const current = await fetch(url, { headers });
    if (!current.ok)
        throw new Error(`clearMutedConversations: GET ${current.status}`);
    const { notification_prefs: prefs } = (await current.json());
    const res = await fetch(url, {
        method: "PUT",
    ...
    if (!res.ok)
        throw new Error(`clearMutedConversations: PUT ${res.status}`);
}
...
// shape isn't read.
export async function joinChannel(token, networkSlug, channelName) {
...
export async function listChannelNames(token, networkSlug) {
    const url = `${GRAPPA_BASE_URL}/networks/${encodeURIComponent(networkSlug)}/channels`;
    const res = await fetch(url, { headers: { authorization: `Bearer ${token}` } });
    if (!res.ok) {
...
export async function patchNetworkConnectionState(token, networkSlug, body) {
    ...
    if (!res.ok) {
        throw new Error(`patchNetworkConnectionState: ${networkSlug} → ${res.status} ${await res.text()}`);
    }
}
...
export async function settleNetworkAutojoin(token, networkSlug, channel, ownNick) {
    await patchNetworkConnectionState(token, networkSlug, {
        connection_state: "connected",
    }).catch(() => { });
    const headers = { authorization: `Bearer ${token}` };
    const channelsUrl = `${GRAPPA_BASE_URL}/networks/${networkSlug}/channels`;
    const membersUrl = `${GRAPPA_BASE_URL}/networks/${networkSlug}/channels/${encodeURIComponent(channel)}/members`;
        ...
        if (res?.ok) {
            ...
            if (channels.find((c) => c.name === channel)?.joined) {
                ...
                if (membersRes?.status === 200) {
                    ...
                    if (members.some((m) => m.nick === ownNick))
                        return;
                }
            }
        }
...
export async function setNetworkIdentityNick(token, slug, nick) {
...
export async function accreteNetwork(token, slug) {
...
export async function getReadCursor(token, networkSlug, channel) {
...
export async function resetSubject(adminToken, userName, baselineAutojoin, baselineSeed) {
    const body = { user_name: userName };
    if (baselineAutojoin)
        body.baseline_autojoin = baselineAutojoin;
    if (baselineSeed) {
        body.baseline_seed = Object.fromEntries(Object.entries(baselineSeed).map(([slug, chans]) => [
            slug,
            chans.map((c) => ({
                name: c.name,
...
// in the e2e seeder).
export async function setCredentialAutojoin(adminToken, userId, networkId, autojoinChannels) {
 ...
 */
export async function setUploadConfirmEnabled(token, enabled) {
    ...
    if (!response.ok) {
        throw new Error(`setUploadConfirmEnabled(${enabled}) failed: ${response.status} ${await response.text()}`);
    }
}
/**
 ...
 */
export async function setShowEventBadge(token, on) {
    const url = `${GRAPPA_BASE_URL}/me/settings/display-prefs`;
    const headers = { "content-type": "application/json", authorization: `Bearer ${token}` };
    const current = await fetch(url, { headers });
    if (!current.ok) {
        throw new Error(`setShowEventBadge(${on}): GET display-prefs → ${current.status} ${await current.text()}`);
    }
    ...
    if (!written.ok) {
        throw new Error(`setShowEventBadge(${on}): PUT display-prefs → ${written.status} ${await written.text()}`);
    }
}
