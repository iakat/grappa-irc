...
export async function snapshotBundle() {
    ...
    return {
        restore: async () => {
            const errors = [];
            const distEntries = await fs.readdir(DIST_DIR).catch((err) => {
                errors.push(err);
                return [];
            });
            await Promise.all(distEntries
                .filter((e) => e !== ".gitkeep")
                .map(async (e) => {
                try {
            ...
            if (errors.length > 0) {
                throw new Error(`bundleSwap.restore: ${errors.length} cleanup error(s); see console.warn output above`);
            }
        },
    };
}
 ...
 */
export async function readCurrentBundleHash() {
 ...
 */
export async function swapToBundleB() {
    return swapEntryAsset((newHash) => `// UX-6-I.2 synthetic bundle B — fixture-generated.\n` +
 ...
 */
export async function swapToBootableBundleB() {
    const currentHash = await readCurrentBundleHash();
    const entry = await fs.readFile(path.join(DIST_DIR, ASSETS_DIR, `index-${currentHash}.js`), "utf8");
 ...
 * bundle B contains.
 */
async function swapEntryAsset(contentsFor) {
 ...
 */
export async function swapServiceWorker() {
...
async function distIndexIsSynthetic() {
    try {
        const html = await fs.readFile(path.join(DIST_DIR, INDEX_HTML), "utf8");
...
async function replaceDistFromSnapshot() {
