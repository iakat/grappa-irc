...
const measureFields = (card, dlSelector) => card.evaluate((el, sel) => {
    const dl = el.querySelector(sel);
    if (dl === null)
        throw new Error(`card has no ${sel}`);
    const rows = [];
    let label = "";
 ...
 * line.
 */
export const expectRailFieldsStacked = async (card, dlSelector) => {
    ...
    for (const row of fields.rows) {
        expect(row.width, `rail ${row.tag} "${row.label}" is ${row.width}px of the card's ${fields.dlWidth}px — still two columns`).toBeGreaterThanOrEqual(fields.dlWidth - 0.5);
        expect(row.lines, `rail ${row.tag} "${row.label}" (${row.len} chars) wraps onto ${row.lines} lines`).toBeLessThanOrEqual(Math.max(1, Math.ceil(row.len / MIN_CHARS_PER_LINE)));
    }
};
/**
 ...
 * here too.
 */
export const expectFieldsTwoColumn = async (card, dlSelector) => {
    const fields = await measureFields(card, dlSelector);
    ...
    for (const row of labels) {
        expect(row.width, `overlay DT "${row.label}" spans the whole card — the rail override leaked`).toBeLessThan(fields.dlWidth);
    }
};
