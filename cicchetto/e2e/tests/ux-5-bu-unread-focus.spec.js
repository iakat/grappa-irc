...
test("focus-regain clears mention badge on selected+blurred window (BU bug-1)", async ({ page, }) => {
    const vjt = specUser();
...
test("re-clicking active sidebar row does NOT POST ReadCursor (BU bug-2 idempotency)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
