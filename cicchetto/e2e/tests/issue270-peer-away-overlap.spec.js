...
import { composeSend, loginAs, scrollbackLine, selectChannel, sidebarWindow, waitForDmListenerReady, } from "../fixtures/cicchettoPage";
...
test("#270 — peer-away banner does not overlap the first DM message row", async ({ page }) => {
    ...
    try {
        ...
        await expect
            ...
            message: "peer-away banner must NOT overlap the first DM row (the floating overlay paints on top of the y=0 first row)",
            timeout: 5_000,
        })
            .toBe(0);
        ...
        if (bb && rb) {
            expect(overlapArea(bb, rb), `overlap must be 0 — banner ${fmt(bb)} vs first row ${fmt(rb)}`).toBe(0);
            expect(rb.y, `first DM row top ${rb.y.toFixed(0)} must sit at/below banner bottom ${(bb.y + bb.height).toFixed(0)}`).toBeGreaterThanOrEqual(bb.y + bb.height - 0.5);
        }
    }
    finally {
