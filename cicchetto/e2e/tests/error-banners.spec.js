...
async function tripWsUnhealthy(page, code, reason) {
...
test("WS-health source renders the generic close-code entry, auto-dismisses on open", async ({ page, }) => {
...
test("a dismissed banner re-arms and re-appears after the source recovers + re-fires (#207)", async ({ page, }) => {
    await tripWsUnhealthy(page, 1006, "");
...
test("WS-health generic entry surfaces the real close code + reason for non-1006", async ({ page, }) => {
...
test("sw-registration source surfaces the captured error name + message (#120)", async ({ page, }) => {
    ...
    await page.evaluate(() => {
        ...
        if (!sw)
            throw new Error("__cic_swRegistration hook missing");
        sw.recordError({
            name: "SecurityError",
...
test("two distinct error sources STACK vertically without overlapping", async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        if (bh.bootHash() === null)
            throw new Error("boot hash null — not a built surface");
    ...
    if (!wsBox || !bundleBox)
        throw new Error("banner slots have no layout box");
