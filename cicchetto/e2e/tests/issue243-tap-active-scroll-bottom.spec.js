...
// re-tap surfaces.
async function scrollUpThenRetapLandsAtBottom(page) {
    const vjt = specUser();
...
test.describe("#243 — re-tap active channel jumps scrollback to bottom", () => {
    test("desktop: re-clicking the active sidebar row jumps to the newest message", async ({ page, }) => {
        await scrollUpThenRetapLandsAtBottom(page);
    });
    test("@webkit @touch mobile: re-tapping the active bottom-bar entry jumps to the newest message", async ({ page, }) => {
        await scrollUpThenRetapLandsAtBottom(page);
    });
});
