...
async function box(locator) {
    ...
    if (b === null)
        throw new Error("boundingBox: element is not rendered");
    return b;
}
test("#275 — channel modes stack below the name in a width-capped box that opens the /mode modal", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        expect(modesBox.y, `modes top ${modesBox.y}px must be at/below the name bottom ${nameBox.y + nameBox.height}px (stacked, not inline)`).toBeGreaterThanOrEqual(nameBox.y + nameBox.height - 2);
        expect(Math.abs(modesBox.x - nameBox.x), `modes left ${modesBox.x}px must align with name left ${nameBox.x}px (same column box), not off on the same row`).toBeLessThan(24);
        ...
        expect(topicBox.width, `topic width ${topicBox.width}px must exceed half the bar ${barBox.width}px`).toBeGreaterThan(barBox.width * 0.5);
