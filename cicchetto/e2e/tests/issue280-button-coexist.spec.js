...
test.describe("#280 — next-active + scroll-to-bottom coexist cleanly", () => {
    test("@webkit @touch — both buttons coexist without overlap, same size, and next-active stays constant relative to the message container across keyboard open", async ({ page, }) => {
        const vjt = specUser();
        ...
        try {
            ...
            if (!naClosed || !sbClosed)
                return;
            expect(overlapArea(naClosed, sbClosed), `keyboard-closed overlap must be 0 — next-active ${fmt(naClosed)} vs scroll-to-bottom ${fmt(sbClosed)}`).toBe(0);
            // Item 4 — same size (the pair must look consistent). scroll-to-
            // bottom used to be a 2rem square vs next-active's 3.5rem circle.
            expect(Math.abs(naClosed.width - sbClosed.width), `widths must match — next-active ${naClosed.width.toFixed(1)} vs scroll-to-bottom ${sbClosed.width.toFixed(1)}`).toBeLessThan(0.6);
            expect(Math.abs(naClosed.height - sbClosed.height), `heights must match — next-active ${naClosed.height.toFixed(1)} vs scroll-to-bottom ${sbClosed.height.toFixed(1)}`).toBeLessThan(0.6);
            ...
            expect(naClosed.y + naClosed.height, `next-active bottom ${(naClosed.y + naClosed.height).toFixed(0)} must sit above scroll-to-bottom top ${sbClosed.y.toFixed(0)}`).toBeLessThanOrEqual(sbClosed.y + 0.6);
            ...
            await expect
                ...
                message: "keyboard-open: next-active offset within the message container must stay constant (pre-fix the 8rem focus-lift jumps it up)",
                timeout: 5_000,
            })
                .toBe(0);
            ...
            if (naOpen && sbOpen) {
                expect(overlapArea(naOpen, sbOpen), `keyboard-open overlap must be 0 — next-active ${fmt(naOpen)} vs scroll-to-bottom ${fmt(sbOpen)}`).toBe(0);
            }
        }
        finally {
    ...
    test("@webkit @touch — badge is BLUE (normal class) when the next target is a plain channel message", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("@webkit @touch — badge is RED (priority class) when the next target carries a mention", async ({ page, }) => {
        const vjt = specUser();
