...
import { findUserIdByName, GRAPPA_BASE_URL, listSessionLogSessions, mintVisitor, reapVisitors, setAdminFlag, } from "../fixtures/grappaApi";
...
function adminSurfaces(vjtUserId) {
    return [
        ...ADMIN_TABS.map((tab) => ({
            name: `tab:${tab}`,
            open: (page) => openTab(page, tab),
        })),
        {
...
async function createSeedVhost(adminToken) {
    const idx = await fetch(`${GRAPPA_BASE_URL}/admin/vhosts`, {
        headers: { authorization: `Bearer ${adminToken}` },
    });
    if (!idx.ok)
        throw new Error(`GET /admin/vhosts → ${idx.status}`);
    ...
    if (!res.ok)
        throw new Error(`POST /admin/vhosts ${address} → ${res.status} ${await res.text()}`);
...
async function seedEndedSession(adminToken) {
    const visitor = await mintVisitor(`ux6g-ended-${Date.now()}`);
    const prefix = `visitor:${visitor.id}:`;
    await expect
        .poll(async () => (await listSessionLogSessions(adminToken)).filter((e) => e.session_id.startsWith(prefix))
        .length, {
        timeout: 20_000,
...
async function deleteSeedVhost(adminToken, id) {
    if (id === null)
        return;
...
test.describe("UX-6-G — admin pane horizontal scroll on mobile", () => {
    ...
    test("@webkit @touch admin on mobile — no surface can be panned sideways", async ({ page }) => {
        ...
        try {
            ...
            await expect(page.getByTestId("admin-pane").locator("[role='tab']"), "a tab exists that the surface list does not name — add it to ADMIN_TABS").toHaveCount(ADMIN_TABS.length);
            const offenders = [];
            for (const surface of adminSurfaces(vjtUserId)) {
                ...
                const found = await page.getByTestId("admin-pane").evaluate((root) => {
                    ...
                    const consider = (el) => {
                        ...
                        if (el.classList.contains("adm-nav"))
                            return;
            ...
            expect(offenders, `nothing in the admin pane may be pannable at 393px, on any surface — ` +
                `offenders: ${JSON.stringify(offenders, null, 2)}`).toEqual([]);
        }
        finally {
    ...
    test("@webkit @touch admin on mobile — the tab strip is one row, and it pans", async ({ page, }) => {
        await openAdminPane(page);
        const strip = await page.getByTestId("admin-pane").evaluate((pane) => {
            const nav = pane.querySelector(".adm-nav");
            if (nav === null)
                throw new Error(".adm-nav is gone — the tab strip's class drifted");
        ...
        expect(strip.rows, `the tab strip must stay on one row at 393px — chips found on ${strip.rows} rows`).toBe(1);
        ...
        expect(strip.overflowX, "the tab strip must be able to pan — that is what the h-scroll exemption is for").toMatch(/auto|scroll/);
        expect(strip.scrollW, `the tab strip must actually overflow, or the exemption guards nothing ` +
            `(scrollWidth ${strip.scrollW} vs clientWidth ${strip.clientW})`).toBeGreaterThan(strip.clientW + 1);
    });
    test("@webkit @touch admin on mobile — vertical scroll inside the pane still works", async ({ page, }) => {
        await openAdminPane(page);
    ...
    test("@webkit @touch admin on mobile — opening a row's detail does not move the row", async ({ page, }) => {
        await openAdminPane(page);
        ...
        // which link moved.
        const scrollChain = () => expander.evaluate((el) => {
            const out = {};
        ...
        expect(Math.abs((after?.y ?? 0) - (before?.y ?? 0)), `the row an operator tapped must not move when its detail opens — ` +
            `scrollable ancestors ${JSON.stringify(scrollBefore)} → ${JSON.stringify(scrollAfter)}: ` +
            `all unchanged means the LAYOUT moved, any change means the tap scrolled`).toBeLessThanOrEqual(1);
    });
});
