...
import { openRailMenu, waitForChannelReady, waitForScrollbackRefreshed, } from "../fixtures/cicchettoPage";
...
test.describe("issue #281 — account switch replay", () => {
    test("switching account A → B fires NO history/featured fetch for A's network", async ({ page, }) => {
        ...
        await page.addInitScript(() => {
            ...
            const bearerTail = (input, init) => {
                const headers = init?.headers ?? (typeof input === "object" && "headers" in input ? input.headers : null);
                if (!headers)
                    return null;
        ...
        }, [a.token, JSON.stringify(a.subject)]);
        await page.goto("/");
        ...
        const clearedAt = await page.evaluate(() => {
            window.__cic281Requests.length = 0;
        ...
        const forensics = await page.evaluate((slug) => {
            ...
            return {
                offending: w.__cic281Requests.filter((r) => {
                    ...
                    const isAMessages = path.includes(`/networks/${slug}/channels/`) && path.includes("/messages");
                    const isAFeatured = path === `/networks/${slug}/featured` || path.startsWith(`/networks/${slug}/featured?`);
        ...
        expect(forensics.offending, `account switch replayed account A's fetches under B's session.\n${evidence}`).toEqual([]);
    });
});
