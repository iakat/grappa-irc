...
// real regression.
import { composeSend, composeTextarea, loginAs, selectChannel, synthSwipe, } from "../fixtures/cicchettoPage";
...
async function probeVerticalClaim(page, args) {
    return await page.evaluate(({ scrollTop, dir }) => {
        ...
        const touch = (y) => new Touch({ identifier: 1, target: ta, clientX: 100, clientY: y });
        const fire = (type, y) => {
            const t = touch(y);
            return ta.dispatchEvent(new TouchEvent(type, {
                bubbles: true,
                cancelable: true,
                touches: [t],
                targetTouches: [t],
...
async function probeHandoff(page, args) {
...
test("issue123 — fast swipe-up recalls history, slow drag does not (touchend velocity gate)", async ({ page, }) => {
    if (!CHANNEL)
...
test("issue123 — nested-scroll handoff: native scroll owns the drag until the edge, then the gesture claims (both directions)", async ({ page, }) => {
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
    await loginAs(page, vjt);
    ...
    const geom = await ta.evaluate((el) => ({
...
test("@webkit @touch issue123 — compose textarea is touch-action: pan-y + overscroll-behavior: contain", async ({ page, }) => {
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
    await loginAs(page, vjt);
