...
const IPAD_PRO_11_PORTRAIT = { width: 834, height: 1194 };
async function boxWidth(page, selector) {
...
async function assertServerRailCapped(page) {
    await page.route((url) => url.pathname === "/networks", async (route) => {
        const res = await route.fetch();
        const rows = (await res.json());
        const patched = rows.map((n) => n.connection ? { ...n, connection: { ...n.connection, server: LONG_IPV6 } } : n);
...
test.describe("#605 server-window rail width cap", () => {
    ...
    test("a long connection.server can't starve the centre on an 834px viewport", async ({ page, }) => {
...
test.describe("#605 server-window rail width cap (webkit)", () => {
    ...
    test("@webkit @touch a long connection.server can't starve the centre on WebKit", async ({ page, }) => {
        await assertServerRailCapped(page);
    });
});
