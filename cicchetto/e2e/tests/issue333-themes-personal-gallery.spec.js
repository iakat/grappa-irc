...
test.describe("#333 — themes personal/gallery + copy UX + delete confirm", () => {
    test("copy creates a personal section (before gallery) and scrolls to it", async ({ page }) => {
        ...
        try {
            ...
            if (personalBox === null || galleryBox === null)
                throw new Error("section has no box");
    ...
    test("deleting an owned theme goes through the confirm modal (cancel keeps, confirm removes)", async ({ page, }) => {
        const admin = getSeededAdmin();
