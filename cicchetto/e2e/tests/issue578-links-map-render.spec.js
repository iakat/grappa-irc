...
test("#578 — /links map: no overlapping labels, nothing clipped, legend + you-marker", async ({ page, }) => {
    const vjt = specUser();
    ...
    for (let i = 0; i < labelRects.length; i++) {
        for (let j = i + 1; j < labelRects.length; j++) {
            ...
            if (a === undefined || b === undefined)
                continue;
            expect(boxesOverlap(a, b), `labels ${i} and ${j} overlap — the declutter/anchor fix regressed`).toBe(false);
        }
    }
