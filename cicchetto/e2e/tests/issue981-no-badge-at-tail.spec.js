...
async function armBadgeObserver(page) {
    await page.evaluate((selector) => {
        ...
        const sample = () => {
            for (const el of document.querySelectorAll(selector)) {
                ...
                if (text !== "")
                    window.__cic981Sightings?.push({ text, atMs: Date.now() });
            }
        };
        sample();
...
async function readSightings(page) {
    return await page.evaluate(() => window.__cic981Sightings ?? []);
}
async function clearSightings(page) {
    await page.evaluate(() => {
        window.__cic981Sightings = [];
    });
}
test.describe("#981 badge on a window read at the tail", () => {
    test("never shows, not even for an instant, while the operator watches the tail", async ({ page, }) => {
        if (!CHANNEL)
        ...
        try {
            ...
            expect(sightings, `badge appeared while the pane sat at the tail: ${JSON.stringify(sightings)}`).toEqual([]);
            ...
            expect(control.length, "the MutationObserver recorded nothing for a badge Playwright can see — the negative above is not evidence").toBeGreaterThan(0);
        }
        finally {
