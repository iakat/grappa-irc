...
test("@webkit iOS — keep-keyboard duration-gates its mousedown preventDefault on scrollback: short tap not prevented (keyboard closes), long-press prevented (selection kept), chrome always prevented", async ({ page, }) => {
    const vjt = specUser();
    ...
    const outcome = await page.evaluate(async ({ body, holdMs }) => {
        ...
        const rows = Array.from(document.querySelectorAll('[data-testid="scrollback-line"][data-kind="privmsg"]'));
        ...
        if (scrollbackRow === null || sendButton === null) {
            return { activeIsTextarea, isIos, missing: true };
        }
