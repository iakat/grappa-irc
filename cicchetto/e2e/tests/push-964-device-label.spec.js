...
test("twins carry ordinals, a rename replaces one, and the survivor drops its number", async ({ page, browser, }) => {
    ...
    try {
        ...
        await expect(observer.locator('[data-testid="devices-list"] li').filter({ hasText: "il portatile" })).toHaveCount(1);
    }
    finally {
 ...
 * makes it.
 */
async function registerDevice(token, userAgent, endpoint) {
    const res = await fetch(`${GRAPPA}/push/subscriptions`, {
        ...
        body: JSON.stringify({
            endpoint,
            keys: {
                p256dh: "BCfaYE5dGabdzef68MI0SN24b4Gsf1t_N3ftUlWaFGzkuudjHLor0CRjosM3c7SLZ7PfFufpsFUh8vsO1t8wCHs",
