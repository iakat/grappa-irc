...
async function setVisualViewportHeight(page, px) {
    await page.evaluate((h) => {
        const vv = window.visualViewport;
        if (!vv)
            throw new Error("#245 spec: window.visualViewport unavailable");
...
test("@webkit @touch #245 — .scrollback re-measures overflow (touch-action) on visualViewport resize", async ({ page, }) => {
    ...
    await resetSubject(admin.token, specUser().name, { [NETWORK_SLUG]: AUTOJOIN_CHANNELS }, { [NETWORK_SLUG]: [{ name: CHANNEL, seedCount: SMALL_SEED_COUNT, seedSender: "seed-bot" }] });
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
    const scrollback = page.getByTestId("scrollback");
    await expect(scrollback).toBeVisible({ timeout: 10_000 });
    const touchAction = () => scrollback.evaluate((el) => getComputedStyle(el).touchAction);
