...
async function wheelScrollbackToTop(page) {
...
test.describe("CP14 B2 — scroll-up triggers loadMore (no end-of-history bounce)", () => {
    ...
    test("scroll-to-top fetches older rows and preserves user's scroll position", async ({ page, }) => {
        const vjt = specUser();
        ...
        if (!tail)
            throw new Error("cp14-b2: seeded #spec-wN corpus is empty");
