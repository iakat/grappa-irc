...
import { composeTextarea, loginAs, scrollbackDistanceFromBottom, scrollbackLines, selectChannel, } from "../fixtures/cicchettoPage";
...
async function scrollChannelUp(page) {
    await expect
...
async function surfaceBothFloatButtons(page) {
    const vjt = specUser();
...
test.describe("#289 — mobile floating buttons are translucent (text shows through)", () => {
    test("@webkit @touch — next-active + scroll-to-bottom are translucent yet clearly tappable", async ({ page, }) => {
        const vjt = specUser();
        ...
        try {
            ...
            expect(naOpacity, `next-active must be translucent (< ${OPAQUE}) so text behind stays legible — got ${naOpacity}`).toBeLessThan(OPAQUE);
            expect(naOpacity, `next-active must stay clearly tappable (>= ${TAPPABLE_FLOOR}) — got ${naOpacity}`).toBeGreaterThanOrEqual(TAPPABLE_FLOOR);
            expect(sbOpacity, `scroll-to-bottom must be translucent (< ${OPAQUE}) so text behind stays legible — got ${sbOpacity}`).toBeLessThan(OPAQUE);
            expect(sbOpacity, `scroll-to-bottom must stay clearly tappable (>= ${TAPPABLE_FLOOR}) — got ${sbOpacity}`).toBeGreaterThanOrEqual(TAPPABLE_FLOOR);
            ...
            expect(naOpacity, `#302: next-active must be MORE translucent (<= ${TRANSLUCENT_CEIL}) — got ${naOpacity}`).toBeLessThanOrEqual(TRANSLUCENT_CEIL);
            expect(sbOpacity, `#302: scroll-to-bottom must be MORE translucent (<= ${TRANSLUCENT_CEIL}) — got ${sbOpacity}`).toBeLessThanOrEqual(TRANSLUCENT_CEIL);
        }
        finally {
...
test.describe("#302 — mobile float buttons don't latch :hover 'selected' after tap", () => {
    test("@webkit @touch — hover-less pointer keeps next-active + scroll-to-bottom at base (no latch)", async ({ page, }) => {
        const vjt = specUser();
        const { peer, bgChannel } = await surfaceBothFloatButtons(page);
        try {
            ...
            expect(hoverNone, "webkit-iphone-15 must emulate a hover-less pointer for this contract to witness the fix").toBe(true);
            ...
            expect(naHovered, `#302: next-active must NOT latch the accent 'selected' fill on a hover-less pointer (base ${naBase}, hovered ${naHovered})`).toBe(naBase);
            ...
            expect(sbHovered, `#302: scroll-to-bottom must NOT brighten-latch on a hover-less pointer (base ${sbBase}, hovered ${sbHovered})`).toBeCloseTo(sbBase, 2);
        }
        finally {
