...
test("#500 — an overflowing member list keeps the rail launcher reachable and openable", async ({ page, }) => {
...
test("#588 — an overflowing rail menu keeps its topmost action inside the viewport", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
