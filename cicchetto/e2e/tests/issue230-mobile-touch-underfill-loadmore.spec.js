...
async function synthTouchDrag(page, startY, endY) {
...
async function seedCursorToHead(token, channel) {
    const asc = await fetchAllMessagesAsc(token, NETWORK_SLUG, channel);
    const head = asc[asc.length - 1];
    if (!head)
        throw new Error("#spec-wN seed corpus empty — cannot seed read cursor to head");
...
test.describe("issue #230 (mobile) — touch-drag-down loads older history when content underfills", () => {
    ...
    test("@webkit @touch issue230 mobile — underfilled .scrollback is touch-action: none + overscroll-behavior: contain", async ({ page, }) => {
        const vjt = specUser();
        if (!CHANNEL)
            throw new Error("AUTOJOIN_CHANNELS empty");
        await seedCursorToHead(vjt.token, CHANNEL);
