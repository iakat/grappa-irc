...
test("#169 — /who renders the WhoModal with parsed rows; scrollback stays clean; nick opens a query", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(scrollbackLine(page, "notice", `[${CHANNEL}]`).filter({ hasText: peer.nick })).toHaveCount(0);
