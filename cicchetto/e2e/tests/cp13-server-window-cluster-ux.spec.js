...
import { composeTextarea, loginAs, selectChannel, sidebarMessageBadge, sidebarWindow, } from "../fixtures/cicchettoPage";
...
test.describe("CP13 server-window cluster", () => {
    test("S6 — bottom numeric-inline pane is gone (no `.numeric-inline-pane` in DOM)", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("S9 — ComposeBox renders on $server + slash-only gate rejects plain text", async ({ page, }) => {
        const vjt = specUser();
        await loginAs(page, vjt);
        ...
        await expect(page.locator(".compose-box-error")).toContainText("Server window accepts only slash-commands");
        // Draft preserved on rejection.
        await expect(ta).toHaveValue("this is not a slash command");
    });
    test("S8 — $server window surfaces unread message badge after live numeric arrives", async ({ page, }) => {
        const vjt = specUser();
        await loginAs(page, vjt);
