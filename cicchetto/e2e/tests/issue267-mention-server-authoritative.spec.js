...
const mentionBody = (runId) => `${specNick()}: server-authoritative mention ${runId}`;
...
async function assertJoinedWithOwnMember(page) {
...
test("#267 — mention landing during a WS gap surfaces from the server count on reconnect (never a client bump)", async ({ page, }) => {
    const vjt = specUser();
...
test("#267 — mention on an unfocused channel fans out from the server to both tabs' badges", async ({ browser, }) => {
    const vjt = specUser();
    const runId = crypto.randomUUID().slice(0, 8);
    const body = mentionBody(runId);
