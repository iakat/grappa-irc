...
test.describe("#947 — the unread divider counts the conversation, not the page", () => {
    ...
    test("jumping back into the unread region keeps the count the affordance advertised", async ({ page, }) => {
        if (!CHANNEL)
        ...
        if (!lastReadRow)
            throw new Error("#947 spec: seeded #spec-wN rows missing cursor index");
        ...
        const flow = await page
            ...
            .evaluateAll((nodes) => nodes.map((n) => n.getAttribute("data-testid") === "unread-marker"
            ? "MARKER"
        ...
        if (!firstUnread)
            throw new Error("#947 spec: no row after the planted cursor");
