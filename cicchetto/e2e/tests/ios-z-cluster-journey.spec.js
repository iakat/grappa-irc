...
import { confirmModal, confirmModalYes, loginAs, openSettingsSection, selectChannel, sidebarCloseButton, sidebarWindow, } from "../fixtures/cicchettoPage";
...
test("@webkit @touch iOS-Z cluster — viewport + safe-area + close× + font-size", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        const xlSize = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue("--font-size").trim());
        ...
        await openSettingsSection(page, "display");
        await expect(page.locator('[data-testid="font-size-XL"]')).toBeChecked();
        const reloadedSize = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue("--font-size").trim());
