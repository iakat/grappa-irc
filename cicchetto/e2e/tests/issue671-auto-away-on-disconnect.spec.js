...
function isVisibilityReport(message) {
...
test("#671 — a stale-then-dead last socket still drives the bouncer AWAY (peer observes it)", async ({ page, }) => {
    ...
    await page.routeWebSocket(/\/socket\/websocket/, (ws) => {
        ...
        ws.onMessage((message) => {
            if (dropVisibility && isVisibilityReport(message))
                return;
            server.send(message);
        });
    ...
    try {
        ...
        await page.waitForFunction(() => window.__visibilityAck === true, null, { timeout: 10_000 });
        ...
        await page.evaluate(() => window.__cic_dropSocketForTests?.());
        ...
        const awaySeen = peer.waitForLine(new RegExp(` 301 .* ${specNick()} `), `RPL_AWAY for ${specNick()}`, 25_000);
        peer.whois(specNick());
