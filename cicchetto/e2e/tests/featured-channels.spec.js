...
test("admin adds a featured channel to a network via the disclosure", async ({ page }) => {
    ...
    try {
        ...
        await expect(page.locator(`[data-testid^='admin-network-featured-row-${slug}-']`)).toHaveCount(1, { timeout: 5_000 });
    }
    finally {
        if (networkId !== null)
...
test("admin deletes a featured channel via inline-confirm", async ({ page }) => {
    ...
    try {
        ...
        await delBtn.click();
        await expect(page.locator(`[data-testid='admin-network-featured-row-${slug}-${featuredId}']`)).toHaveCount(0, { timeout: 5_000 });
