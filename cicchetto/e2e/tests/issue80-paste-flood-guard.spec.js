...
import { composeTextarea, confirmModal, confirmModalAlternative, confirmModalBody, confirmModalCancel, confirmModalYes, loginAs, scrollbackLine, selectChannel, } from "../fixtures/cicchettoPage";
...
test("#80 — multi-line paste: dialog opens, Cancel drops it, Paste inserts it", async ({ page, }) => {
    if (!CHANNEL)
...
test("#816 — a paste over the hard cap uploads as a file instead of flooding", async ({ page, browserName, }) => {
    test.skip(browserName !== "chromium", "shares the upload plumbing gated to chromium by the uploads specs; the paste half is browser-agnostic and covered above");
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
    await loginAs(page, vjt);
    ...
    await page.evaluate(() => localStorage.setItem("image-upload-privacy-acknowledged:embedded", "1"));
    const ta = composeTextarea(page);
    await expect(ta).toBeVisible();
    await expect(ta).toHaveValue("");
...
test("#816 — an under-cap paste offers the .txt upload beside Paste, and it works", async ({ page, browserName, }) => {
    test.skip(browserName !== "chromium", "shares the upload plumbing gated to chromium by the uploads specs");
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
    await loginAs(page, vjt);
