...
test("P-0c — /whowas after peer disconnect surfaces WhowasCard with historical fields", async ({ page, }) => {
    const vjt = specUser();
...
test("P-0c — /whowas against unknown nick surfaces 'no history' (406 ERR_WASNOSUCHNICK)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
