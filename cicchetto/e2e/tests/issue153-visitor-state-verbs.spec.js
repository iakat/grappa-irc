...
import { bootVisitorContext, composeSend, expectShellReady, scrollbackLine, selectChannel, waitForUserTopicReady, } from "../fixtures/cicchettoPage";
...
test("issue #153 — visitor /quote and /mode reach upstream and take effect", async ({ browser, }) => {
    ...
    try {
        ...
        const privmsgWitness = peer.waitForLine(new RegExp(`PRIVMSG ${channel} :.*${marker}`, "i"), `visitor /quote PRIVMSG ${marker}`);
        ...
        const modeWitness = peer.waitForLine(new RegExp(`MODE ${channel} \\+m`, "i"), `visitor /mode ${channel} +m`);
        ...
        if (peer)
            await peer.disconnect("issue153 done").catch(() => { });
        await ctx.close();
