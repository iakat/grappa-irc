...
import { loginAs, selectChannel, sidebarMessageBadge, sidebarWindow, waitForDmListenerReady, } from "../fixtures/cicchettoPage";
...
async function seedTwoTierUnread(page, token, peerNick) {
...
test("desktop: Alt+A / button jumps DM (tier 0) before channel (tier 1), then auto-hides", async ({ page, }) => {
    const vjt = specUser();
...
test("@webkit @touch mobile: bottom-bar affordance jumps DM before channel, then auto-hides", async ({ page, }) => {
    const vjt = specUser();
