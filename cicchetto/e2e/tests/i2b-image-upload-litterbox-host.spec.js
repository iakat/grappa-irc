...
function waitForServerSettingsFrame(page, expectedHost) {
    return page.waitForEvent("websocket", { timeout: 10_000 }).then(async (ws) => {
        // Subsequent frames after the WS opens.
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => reject(new Error(`server_settings_changed(${expectedHost}) frame not seen in 10s`)), 10_000);
            ws.on("framereceived", ({ payload }) => {
                if (typeof payload !== "string")
                    return;
                if (!payload.includes("server_settings_changed"))
                    return;
                if (!payload.includes(`"active_host":"${expectedHost}"`))
                    return;
                clearTimeout(timer);
                resolve();
            });
...
test.describe("I-2 litterbox path (admin-pinned host)", () => {
    ...
    test("privacy modal Cancel does NOT trigger upload", async ({ page }) => {
        ...
        const controlModal = await pickFile(page, {
