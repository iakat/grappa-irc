...
// layer.
async function uploadPng(page, hex, filename) {
    return await page.evaluate(async ([hexBody, name]) => {
        ...
        if (!token)
            throw new Error("missing grappa-token in localStorage");
...
test("REV-G H22 — SW does not intercept /uploads/<slug> navigation (serves image bytes)", async ({ page, }) => {
    const vjt = specUser();
