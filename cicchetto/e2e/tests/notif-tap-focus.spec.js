...
async function coldBootAt(page, vjt, url) {
...
test.describe("#146 — notification tap opens & focuses the matching window", () => {
    test("channel highlight tap focuses the channel window (cold path / openWindow)", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("channel highlight tap focuses the channel window (warm path / SW→page navigate)", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("DM tap opens AND focuses the query window (warm path / SW→page navigate)", async ({ page, }) => {
        const vjt = specUser();
        await loginAs(page, vjt);
