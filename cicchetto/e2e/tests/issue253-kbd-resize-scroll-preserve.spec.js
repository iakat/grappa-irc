...
async function simulateKeyboardOpen(page) {
    await page.evaluate((px) => {
        ...
        if (!vv)
            throw new Error("issue253 spec: window.visualViewport unavailable");
...
test.describe("#253 — keyboard/viewport resize must not yank a scrolled-up reader to the tail", () => {
    ...
    test("@webkit @touch #253 — a scrolled-up reader keeps their scrollTop when the keyboard opens", async ({ page, }) => {
        test.slow();
        const vjt = specUser();
        ...
        const before = await sc.evaluate((el, { bottomThreshold, loadMoreThreshold }) => {
    ...
    test("@webkit @touch #253 — an at-bottom reader stays pinned to the tail when the keyboard opens", async ({ page, }) => {
        test.slow();
        const vjt = specUser();
        await loginAs(page, vjt);
        await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
