...
async function userRowKey(page) {
    ...
    if (testId === null)
        throw new Error("the seeded user row lost its testid");
    return testId.replace("admin-session-row-", "");
}
test("#1157 @webkit @touch a phone gives the row's verbs one control, and it is the menu", async ({ page, }) => {
