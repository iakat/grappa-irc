...
import { bootVisitorContext, loginAs, openRailMenu, selectChannel, sidebarWindow, } from "../fixtures/cicchettoPage";
import { adminDeleteTheme, adminDeleteVisitor, copyTheme, listGalleryThemes, mintVisitor, publishTheme, } from "../fixtures/grappaApi";
...
async function openThemesSubPageMobile(page) {
...
// themes nav row).
async function openThemesSubPageDesktop(page) {
    await openRailMenu(page);
...
async function reopenGalleryDesktop(page) {
...
test.describe("#299 — theme cards (tap-select + progressive disclosure)", () => {
    test("@webkit @touch tapping a card reveals exactly one ≥44px action row", async ({ page }) => {
        ...
        if (box === null)
            throw new Error("copy action has no bounding box");
    ...
    test("a minted visitor copies a built-in and gets manage actions on the owned copy", async ({ browser, }) => {
    ...
    test("a visitor-published theme credits the publish-time nick and it survives reap", async ({ page, }) => {
        ...
        try {
            ...
            if (builtin === undefined)
                throw new Error("no built-in theme in the gallery to copy");
