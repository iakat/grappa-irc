...
test("@webkit @touch UX-3 OCT — viewport meta carries interactive-widget=resizes-content", async ({ page, }) => {
    const vjt = specUser();
...
test("@webkit @touch UX-3 OCT — installViewportHeightTracker writes --viewport-height on <html>", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    ...
    const cssVar = await page.evaluate(() => document.documentElement.style.getPropertyValue("--viewport-height").trim());
...
test("@webkit @touch UX-3 OCT — installScrollPin snaps window back to (0, 0) on programmatic scroll", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
...
test("@webkit @touch UX-3 OCT — .shell-mobile reads var(--viewport-height) with 100dvh fallback", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
