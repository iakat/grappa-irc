...
test.describe("#196 — preview overlay holds scroll across a live message arrival", () => {
    ...
    test("a peer message arriving while the image preview is open must NOT move the message list (desktop)", async ({ page, }) => {
        test.slow();
        if (!CHANNEL)
        ...
        try {
            ...
            const before = await link.evaluate((el) => {
                const row = el.closest(".scrollback-line");
                const scroll = document.querySelector('[data-testid="scrollback"]');
