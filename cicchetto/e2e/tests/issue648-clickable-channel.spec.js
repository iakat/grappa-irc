...
import { confirmModal, confirmModalBody, confirmModalYes, loginAs, scrollbackLine, selectChannel, sidebarWindow, } from "../fixtures/cicchettoPage";
...
const freshChannel = (label) => `#i648${label}-${crypto.randomUUID().slice(0, 6)}`;
...
test("a #channel in scrollback is clickable → confirm → joins and switches (#648)", async ({ page, }) => {
    const vjt = specUser();
...
test("clicking a #channel we're already in switches to it with NO modal (#648)", async ({ page, }) => {
    const vjt = specUser();
