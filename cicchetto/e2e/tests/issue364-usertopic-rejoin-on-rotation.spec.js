...
test("#364 — user-topic events + push verbs survive a token rotation with unchanged identity", async ({ page, }) => {
    const vjt = specUser();
    ...
    await page.evaluate((tok) => {
        window.__cic_setTokenForTests?.(tok);
    }, rotated.token);
