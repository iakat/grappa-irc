...
test.describe("#244 directory tap foregrounds the joined window", () => {
    test("DESKTOP — tapping an unjoined /list row joins it AND foregrounds its window", async ({ page, }) => {
        const vjt = specUser();
        ...
        try {
            ...
            await expect(sidebarWindow(page, NETWORK_SLUG, AUTOJOIN_CHANNELS[0])).toHaveClass(/selected/, { timeout: 10_000 });
            ...
            await expect(sidebarWindow(page, NETWORK_SLUG, AUTOJOIN_CHANNELS[0])).not.toHaveClass(/selected/);
        }
        finally {
    ...
    test("DESKTOP no-steal — a server-originated (automatic) join does NOT foreground; selection stays put", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("MOBILE @webkit @touch — tapping an unjoined /list row foregrounds its window (BottomBar branch)", async ({ page, }) => {
        const vjt = specUser();
