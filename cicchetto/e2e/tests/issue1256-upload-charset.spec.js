...
import { composeTextarea, confirmModal, confirmModalYes, loginAs, selectChannel, } from "../fixtures/cicchettoPage";
...
// upload door (#816).
const LINES = 6;
const accentedBlock = (tag) => Array.from({ length: LINES }, (_, i) => `${tag} riga ${i} — perché è così, più o meno: àèìòù`).join("\n");
...
test("#1256 — a pasted UTF-8 block uploads labelled and renders with its accents intact", async ({ page, browserName, }) => {
    test.skip(browserName !== "chromium", "shares the upload plumbing the uploads specs gate to chromium");
    if (!CHANNEL)
    ...
    const uploadPost = page.waitForResponse((r) => r.url().includes("/api/uploads") && r.request().method() === "POST", { timeout: 20_000 });
