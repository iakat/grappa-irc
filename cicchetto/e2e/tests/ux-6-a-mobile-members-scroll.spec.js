...
test("@webkit @touch ux-6-a — mobile members pane (the actual scroller) asserts touch-action: pan-y + overscroll-behavior: contain", async ({ page, }) => {
    const vjt = specUser();
...
test("@webkit @touch ux-6-a — opening members drawer adds html.overlay-open; closing removes it", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    // Baseline: no overlay open → no class on <html>.
    const hasOverlayClass = () => page.evaluate(() => document.documentElement.classList.contains("overlay-open"));
...
test("@webkit @touch ux-6-a — html.overlay-open suspends root touch-action so gesture-escalation can't reach installScrollPin", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
...
test("@webkit @touch ux-6-a v2 — descendants of .members-pane share the scroller's touch-action: pan-y (universal-selector carve-out)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    ...
    await expect(page.locator(".shell-mobile .shell-members .members-pane li .member-name").first()).toBeVisible({ timeout: 5_000 });
...
test("@webkit @touch ux-6-a v2 — members-pane nick-text renders with no inline color (sigil keeps mode color)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    ...
    await page.getByLabel(/open members sidebar/i).tap();
    await expect(page.locator(".shell-members.open")).toBeVisible({ timeout: 5_000 });
    await expect(page.locator(".shell-mobile .shell-members .members-pane li .nick-text").first()).toBeVisible({ timeout: 5_000 });
...
test("@webkit @touch ux-6-a v2 — .member-name:hover underline is gated on (hover: hover) — no spurious underline on touch-only viewports", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
