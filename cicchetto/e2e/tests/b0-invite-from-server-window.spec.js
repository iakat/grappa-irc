...
test("B0 — /invite from $server window (no channel context) reaches upstream + invite-ack lands", async ({ page, }) => {
    const vjt = specUser();
    ...
    await expect(page.locator(".sidebar-network-section li").filter({ hasText: CHANNEL })).toHaveCount(1, { timeout: 10_000 });
