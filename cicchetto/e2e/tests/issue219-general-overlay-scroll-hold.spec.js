...
test("#219-general — a resize while a covering (non-media) modal is open must NOT snap the list to the bottom, and the position survives close", async ({ page, }) => {
    test.slow();
    if (!CHANNEL)
...
test("#219-general (regression guard) — message-follow is untouched: at the tail with NO overlay, an inbound message scrolls to the bottom", async ({ page, }) => {
    test.slow();
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
