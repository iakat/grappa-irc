...
async function dragOnModal(viewer, dy, lift) {
    return viewer.evaluate((el, opts) => {
        // The vertical component of the computed matrix — what the browser
        // actually resolved, not the inline string we wrote.
        const verticalOffset = () => new DOMMatrixReadOnly(getComputedStyle(el).transform).f;
...
test("#1438 — mid-drag the modal moves by the finger's travel, centering intact (chromium)", async ({ page, }) => {
    test.slow();
    const { viewer } = await openImageViewer(page);
...
test("@webkit @touch #1438 — the IMAGE viewer container re-opens the pan, and still dismisses (iPhone 15)", async ({ page, }) => {
    test.slow();
    const { viewer } = await openImageViewer(page);
