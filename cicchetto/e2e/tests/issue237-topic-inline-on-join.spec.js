...
import { composeSend, loginAs, scrollbackLine, selectChannel, sidebarWindow, topicJoinRow, } from "../fixtures/cicchettoPage";
...
async function topicInlineOnJoinAndChange(page) {
    const vjt = specUser();
...
test("#237 — full channel topic prints inline in scrollback on join and on change", async ({ page, }) => {
    await topicInlineOnJoinAndChange(page);
});
test("#237 @webkit @touch — full channel topic prints inline on the mobile viewport too", async ({ page, }) => {
    await topicInlineOnJoinAndChange(page);
});
