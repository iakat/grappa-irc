...
const deleteNotifyList = (token) => fetch(`${GRAPPA_BASE_URL}/networks/${NETWORK_SLUG}/notify`, {
    method: "DELETE",
    headers: { authorization: `Bearer ${token}` },
}).catch(() => { });
test("#356 — with-arg /notify shows a green auto-dismissing notice; a bad command stays sticky", async ({ page, }) => {
    const vjt = specUser();
    try {
...
test("#356 — bare /notify and bare /hilight open the watch-lists section; home shows none", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, SEED_CHANNEL, { ownNick: specNick() });
...
test("#356 — settings keyword list add + × round-trips against real server state", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, SEED_CHANNEL, { ownNick: specNick() });
