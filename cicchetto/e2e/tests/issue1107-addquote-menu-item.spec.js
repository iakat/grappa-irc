...
const FILLER = "una battuta abbastanza lunga da mandare a capo il compose piu' volte, " +
...
async function openMenuOnRow(page, body) {
...
test("issue1107 — !addquote fills the compose box with the command and the message", async ({ page, }) => {
...
test("issue1107 — picking !addquote sends nothing", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="scrollback-line"]', { hasText: "!addquote" })).toHaveCount(0);
});
...
test.describe("the caret, where the overflow is real", () => {
    ...
    test("issue1107 — !addquote scrolls the overflowing compose box down to the caret", async ({ page, }) => {
