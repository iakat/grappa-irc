...
test("issue #557 — /kill <nick> <reason> ships KILL nick :reason on the raw wire, bare /kill has no colon", async ({ browser, }) => {
    ...
    try {
        ...
        expect(sentRawLines.some((l) => l === "KILL spammer :" || l.toLowerCase().includes("null"))).toBe(false);
