...
import { composeSend, loginAs, selectChannel, waitForQueryWindowReady, } from "../fixtures/cicchettoPage";
import { createThemeWithBuiltinBackground, listBuiltinBackgrounds, setActiveTheme, } from "../fixtures/grappaApi";
...
async function activateWallpaperTheme(token, label, opacity) {
...
async function assertCardCloseWinsItsCorner(page) {
    ...
    try {
        ...
        const overlaps = openerBox.x < closeBox.x + closeBox.width &&
            closeBox.x < openerBox.x + openerBox.width &&
            openerBox.y < closeBox.y + closeBox.height &&
            closeBox.y < openerBox.y + openerBox.height;
        expect(overlaps, "#1051 — the ☰ and the card's ✕ must still share the corner for this guard to mean anything").toBe(true);
...
test("@webkit @touch #1051 — with no background theme the card's ✕ is the topmost element at its own coordinates", async ({ page, }) => {
...
test("@webkit @touch #1051 — with a background theme active the card's ✕ still wins its own corner", async ({ page, }) => {
    const vjt = specUser();
...
test("@webkit @touch #1051 — the wallpaper still paints, and still paints behind the conversation", async ({ page, }) => {
    const vjt = specUser();
    ...
    const control = await createThemeWithBuiltinBackground(vjt.token, `e2e-1051-nobg-${vjt.name}`, null, 1);
    const wallpapered = await createThemeWithBuiltinBackground(vjt.token, `e2e-1051-bg-${vjt.name}`, first.key, 1);
    ...
    await expect(page.locator("html.theme-has-bg")).toHaveCount(0);
    expect((await shoot()).equals(controlLight), "#1051 — the OS-scheme flip must be inert under one theme, or it cannot isolate the wallpaper").toBe(true);
    ...
    expect(withWallpaper.equals(withoutWallpaper), "#1051 — an opaque wallpaper must change what the scrollback area paints").toBe(false);
    ...
    expect((await shoot()).equals(withWallpaper), "#1051 — a message arriving must be visible over the wallpaper, not buried under it").toBe(false);
});
