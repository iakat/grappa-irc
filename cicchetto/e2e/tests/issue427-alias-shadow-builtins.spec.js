...
import { composeSend, composeTextarea, loginAs, openSettingsDrawer, scrollbackLine, selectChannel, } from "../fixtures/cicchettoPage";
...
test("#427 — an alias shadows a builtin end-to-end (/whois → /me takes effect)", async ({ page, }) => {
    const vjt = specUser();
    try {
...
test("#427 — a shadowing alias stays removable from Settings → Alias (binding condition)", async ({ page, }) => {
    const vjt = specUser();
    try {
        await clearAliases(vjt.token);
        await loginAs(page, vjt);
