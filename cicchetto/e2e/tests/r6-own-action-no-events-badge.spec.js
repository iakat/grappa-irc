...
// restoration needed.
import { loginAs, selectChannel, sidebarEventsBadge, sidebarWindow, } from "../fixtures/cicchettoPage";
import { joinChannel, partChannel, restoreReadCursorToTail, setShowEventBadge, } from "../fixtures/grappaApi";
...
test("CP29 R-6 — operator's own /part → /join cycle does NOT raise an unread marker for their own presence rows", async ({ page, }) => {
    const vjt = specUser();
    ...
    await expect(page
        ...
        .last()).toBeVisible({ timeout: 10_000 });
