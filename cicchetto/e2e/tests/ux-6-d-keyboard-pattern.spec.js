...
async function promoteVjtToAdmin() {
...
test.describe("UX-6 D cluster close — iOS PWA keyboard pattern @webkit", () => {
    ...
    test("(b) --vh CSS var is written from visualViewport.height in px", async ({ page }) => {
        await loginAs(page, specUser());
        const vhVar = await page.evaluate(() => document.documentElement.style.getPropertyValue("--vh"));
    ...
    test("(c) --viewport-height legacy CSS var writes from same source", async ({ page }) => {
        await loginAs(page, specUser());
        const vpHeightVar = await page.evaluate(() => document.documentElement.style.getPropertyValue("--viewport-height"));
