...
import { confirmModal, confirmModalBody, confirmModalCancel, confirmModalYes, loginAs, waitForUserTopicReady, } from "../fixtures/cicchettoPage";
import { GRAPPA_BASE_URL, patchNetworkConnectionState, } from "../fixtures/grappaApi";
import { AUTOJOIN_CHANNELS, NETWORK_SLUG } from "../fixtures/seedData";
import { expect, specUser, test } from "../fixtures/test";
const SEED_CHANNEL = AUTOJOIN_CHANNELS[0];
async function fetchNetworkState(token, slug) {
...
test("#283 — Home ConnectedRow Disconnect: confirm modal parks the network, Cancel does not", async ({ page, }) => {
    const vjt = specUser();
    ...
    await expect(parkedRow.getByRole("button", { name: new RegExp(`reconnect ${NETWORK_SLUG}`, "i") })).toBeVisible();
});
