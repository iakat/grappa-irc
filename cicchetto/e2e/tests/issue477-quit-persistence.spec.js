...
async function openLifecycleMenu(page) {
...
async function bootVisitor(browser, registered) {
...
test.describe("issue #477 — quit()/detach route on persistence, not class", () => {
    ...
    test("registered VISITOR (persistent) → the rail offers BOTH detach and quit", async ({ browser, }) => {
    ...
    test("ephemeral VISITOR → the rail offers ONLY quit, and quit lands on /login", async ({ browser, }) => {
