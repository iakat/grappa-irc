...
test("issue203 — fast swipe-up over an EMPTY compose recalls the last sent line (parity with ArrowUp)", async ({ page, }) => {
    if (!CHANNEL)
...
test("issue203 — the recalled line matches keydown ArrowUp exactly (swipe≡arrow parity)", async ({ page, }) => {
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
    await loginAs(page, vjt);
...
test("issue203 — fast swipe-DOWN over an EMPTY compose does NOT recall (#178 gate kept on recall-next)", async ({ page, }) => {
    if (!CHANNEL)
        throw new Error("AUTOJOIN_CHANNELS empty");
    const vjt = specUser();
    await loginAs(page, vjt);
