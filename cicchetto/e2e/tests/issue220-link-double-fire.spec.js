...
test.describe("#220 link-bearing surfaces double-fire", () => {
    ...
    test.beforeEach(async ({ context }) => {
        await context.route("**://example.com/**", (route) => route.fulfill({ status: 200, contentType: "text/html", body: "<html>stub</html>" }));
    });
    test("/list row: tapping a topic link browses (opens popup) and does NOT join; tapping the row body joins", async ({ page, context, }) => {
        const vjt = specUser();
    ...
    test("topic bar: tapping a link opens the topic modal and does NOT navigate; the modal link is a working anchor", async ({ page, context, }) => {
        const vjt = specUser();
