...
async function patchConnectionWithClientId(token, clientId, body) {
...
async function fetchChannelMembers(token, channel) {
    const res = await fetch(`${GRAPPA_BASE_URL}/networks/${NETWORK_SLUG}/channels/${encodeURIComponent(channel)}/members`, {
        headers: { authorization: `Bearer ${token}` },
    });
    if (!res.ok)
