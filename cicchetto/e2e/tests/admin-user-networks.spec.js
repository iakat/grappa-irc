...
function unique(prefix) {
    ...
    if (id.length > 32)
        throw new Error(`unique: "${id}" is ${id.length} chars, slug limit is 32`);
    return id;
}
...
async function openUserPage(page, userId) {
 ...
 */
async function enableNetwork(page, networkId, nick) {
...
async function createUser(token, name) {
    ...
    if (!res.ok)
        throw new Error(`createUser: ${name} → ${res.status}`);
...
async function findUserId(token, name) {
    ...
    if (!res.ok)
        throw new Error(`listUsers → ${res.status}`);
    const body = (await res.json());
    const found = body.users.find((u) => u.name === name);
    if (found === undefined)
        throw new Error(`findUserId: ${name} not in the list`);
    return found.id;
}
async function createNetwork(token, slug) {
    ...
    if (!res.ok)
        throw new Error(`createNetwork: ${slug} → ${res.status}`);
    const body = (await res.json());
    return body.id;
}
async function bindCredential(token, userId, networkId, nick) {
    ...
    if (!res.ok)
        throw new Error(`bindCredential: ${nick} → ${res.status}`);
}
async function unbindBestEffort(token, userId, networkId) {
    try {
...
async function deleteUserBestEffort(token, userId) {
    try {
...
test("an operator creates a user and gives it a network in one flow", async ({ page }) => {
    ...
    try {
        ...
        if (userId !== null)
            await deleteUserBestEffort(admin.token, userId);
    }
});
...
test("admin edits a network (realname change) — the section reports left_alone", async ({ page, }) => {
    const admin = getSeededAdmin();
    ...
    try {
        ...
        await expect(page.getByTestId(`admin-user-network-session-action-${networkId}`)).toContainText("left_alone", { timeout: 10_000 });
    }
    finally {
        if (userId !== null && networkId !== null) {
            await unbindBestEffort(admin.token, userId, networkId);
