...
function inspectCorner(el) {
    const describe = (n) => `${n.tagName.toLowerCase()}${[...n.classList].map((c) => `.${c}`).join("")}` +
...
async function reportCorner(testInfo, label, readings) {
    if (readings.length === 0)
        return;
    for (const r of readings)
        console.log(`#1050 ${label} ${JSON.stringify(r)}`);
...
test("@webkit @touch #1050 — the /list window drops the floating ☰, and its ✕ actually closes the directory", async ({ page, }, testInfo) => {
    const vjt = specUser();
    ...
    expect(probe.inViewport, `#1050 — the directory's ✕ must be inside the viewport. ${JSON.stringify(probe)}`).toBe(true);
    expect(probe.hit, `#1050 — nothing may paint over the directory's close ✕. ${JSON.stringify(probe)}`).toBe(true);
