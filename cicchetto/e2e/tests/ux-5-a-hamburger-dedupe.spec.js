...
test("ux-5-a desktop — ZERO chrome hamburgers; TopicBar hamburger CSS-hidden on desktop", async ({ page, }) => {
    const vjt = specUser();
    ...
    await expect(page.locator(".shell-members .rail-actions-menu [data-testid='action-cluster-cog']")).toBeVisible({
        timeout: 10_000,
    });
...
test("@webkit @touch ux-5-a mobile — exactly ONE visible hamburger (TopicBar members) on channel; ZERO on home", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
