...
import { composeSend, composeTextarea, confirmModal, loginAs, selectChannel, sidebarCloseButton, sidebarWindow, } from "../fixtures/cicchettoPage";
...
for (const m of ESC_MODALS) {
    test(`#232 ${m.name} closes on Esc pressed from body focus (was focus-trapped)`, async ({ page, }) => {
        const vjt = specUser();
...
test("#232 ModeModal: the × button and backdrop click still close it (other doors intact)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
    // × button.
...
test("#232 ConfirmModal: Esc from body focus DISMISSES safely (channel not left)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
