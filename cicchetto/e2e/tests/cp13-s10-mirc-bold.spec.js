...
test("CP13 S10 — peer's bold-formatted PRIVMSG renders with .scrollback-mirc-bold span", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(scrollbackLines(page).locator(".scrollback-mirc-bold", { hasText: boldTag })).toHaveCount(1, { timeout: 10_000 });
    }
    finally {
