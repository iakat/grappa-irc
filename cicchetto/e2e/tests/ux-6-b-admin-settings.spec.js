...
async function openAdminPaneAndSettingsTab(page) {
...
test.describe("UX-6-B admin Settings tab", () => {
    ...
    test("PUT /admin/settings fans out server_settings_changed on user-topics", async ({ page }) => {
        ...
        const [response] = await Promise.all([
            page.waitForResponse((r) => r.url().includes("/admin/settings") && r.request().method() === "PUT"),
    ...
    test("video duration cap: form → PUT → /api/server-settings (#201)", async ({ page, request, }) => {
        const admin = getSeededAdmin();
