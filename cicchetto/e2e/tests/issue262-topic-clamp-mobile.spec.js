...
async function boundingHeight(locator) {
    const box = await locator.boundingBox();
    if (box === null)
        throw new Error("boundingHeight: element has no bounding box (not rendered)");
    return box.height;
}
test("#262/#307 @webkit @touch — a long topic clamps to 2 lines with a native ellipsis (not a bare max-height clip)", async ({ page, }) => {
    const vjt = specUser();
    ...
    const longTopic = `${marker} this is a deliberately very long channel topic used to prove the ` +
    ...
    try {
        ...
        expect(stripHeight, `.topic-bar-topic height ${stripHeight}px must be clamped ≤ ${TOPIC_STRIP_HEIGHT_CAP_PX}px`).toBeLessThanOrEqual(TOPIC_STRIP_HEIGHT_CAP_PX);
        expect(barHeight, `.topic-bar height ${barHeight}px must stay ≤ ${TOPIC_BAR_HEIGHT_CAP_PX}px`).toBeLessThanOrEqual(TOPIC_BAR_HEIGHT_CAP_PX);
        ...
        expect(clampOnlyHeight, `with the max-height backstop removed, -webkit-line-clamp must STILL bound the strip ` +
            `to ~2 lines (${clampOnlyHeight}px) — a dead clamp (the #262 no-ellipsis bug) grows to the full topic`).toBeLessThanOrEqual(50);
    }
    finally {
