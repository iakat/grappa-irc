...
async function readLastBeepAt(page) {
    ...
    return await page.evaluate(() => window.__lastBeepAt ?? null);
}
test("inbound DM fires in-app beep (__lastBeepAt advances) on a non-focused window", async ({ page, }) => {
    const vjt = specUser();
...
test("PRIVMSG without nick mention does NOT fire beep on a non-focused channel", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, AUTOJOIN_CHANNELS[0], { ownNick: specNick() });
    const baseline = await readLastBeepAt(page);
