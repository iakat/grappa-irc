...
async function swipeRow(page, body, points) {
    return await page.evaluate(({ body: text, points: pts }) => {
        ...
        const mk = (p) => new Touch({ identifier: 1, target: row, clientX: p.x, clientY: p.y });
        const fire = (type, p) => {
            const t = mk(p);
            const active = type === "touchend" ? [] : [t];
            return row.dispatchEvent(new TouchEvent(type, {
                bubbles: true,
                cancelable: true,
                touches: active,
        ...
        for (let i = 1; i < pts.length - 1; i++) {
            ...
            if (row.style.transform !== "")
                transformDuring = row.style.transform;
            if (row.classList.contains("scrollback-line-swiping"))
                swipingClassDuring = true;
        }
        fire("touchend", last);
        return {
            prevented,
...
test("issue1067 — a left→right swipe on a message quotes it into the compose box and the row snaps back", async ({ page, }) => {
    ...
    const caret = await composeTextarea(page).evaluate((el) => el.selectionStart);
...
test("issue1067 — a vertical drag on a message never claims the gesture and quotes nothing", async ({ page, }) => {
...
test("issue1067 — a right swipe that STARTS at the left edge opens the sidebar and quotes nothing", async ({ page, }) => {
...
test("issue1067 — a long-press opens the message menu with Copy / Reply / Select…, keyboard closed AND open", async ({ page, }) => {
...
test("issue1067 — the menu's Reply item fills the compose with the same quote the swipe does", async ({ page, }) => {
...
test("issue1067 — the menu's Copy item writes the whole rendered row to the pasteboard", async ({ page, }) => {
    const body = uniqueBody("copy");
    await postMessage(page, body);
    await page.evaluate(() => {
        const written = [];
        window.__copied = written;
    ...
    await expect
        .poll(async () => await page.evaluate(() => window.__copied))
        .toHaveLength(1);
...
test("issue1067 — Select… hands back a live selection over the row and arms the callout re-enable", async ({ page, }) => {
...
test("issue1156 — a join row refuses the swipe while the privmsg above it still quotes", async ({ page, }) => {
...
test("issue1156 — a long press on that join row still opens the menu, Reply disabled", async ({ page, }) => {
