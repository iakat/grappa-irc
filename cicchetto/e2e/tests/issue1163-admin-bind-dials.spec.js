...
async function seededNetworkId(token) {
    const res = await api(token, "GET", "/admin/networks");
    if (!res.ok)
        throw new Error(`seededNetworkId: ${res.status}`);
    const body = (await res.json());
    const net = body.networks.find((n) => n.slug === NETWORK_SLUG);
    if (net === undefined)
        throw new Error(`seededNetworkId: no network ${NETWORK_SLUG}`);
    return net.id;
}
...
async function unbind(token, userId, networkId, testInfo) {
    try {
...
async function deleteUser(token, userId, testInfo) {
    try {
...
test("admin binds a credential from the console and the session dials out", async ({ page, }, testInfo) => {
    ...
    try {
        ...
        await expect(page.getByTestId(`admin-user-network-connection-${networkId}`)).toHaveText("connected");
    }
    finally {
        if (userId !== null) {
