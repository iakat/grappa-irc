...
function isRound(borderRadius, width) {
    const first = borderRadius.split(" ")[0];
    if (first.endsWith("%"))
        return Number.parseFloat(first) >= 50;
    return Number.parseFloat(first) >= width / 2 - 1;
}
test("#264 @webkit @touch — mobile next-active button is a keyboard-safe circle (≥44px, corner badge)", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        expect(Math.abs(unfocused.width - unfocused.height), `circle must be symmetric: width ${unfocused.width}px vs height ${unfocused.height}px`).toBeLessThanOrEqual(SQUARE_TOLERANCE_PX);
        expect(unfocused.width, `width ${unfocused.width}px must be ≥ ${MIN_TAP_PX}px (HIG)`).toBeGreaterThanOrEqual(MIN_TAP_PX);
        expect(unfocused.height, `height ${unfocused.height}px must be ≥ ${MIN_TAP_PX}px (HIG)`).toBeGreaterThanOrEqual(MIN_TAP_PX);
        expect(isRound(unfocused.borderRadius, unfocused.width), `border-radius ${unfocused.borderRadius} must render the ${unfocused.width}px box as a circle`).toBe(true);
        ...
        expect(unfocused.countPosition, `count must be an absolutely-positioned corner badge, got position: ${unfocused.countPosition}`).toBe("absolute");
