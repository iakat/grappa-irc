...
test("full mIRC: peer PRIVMSG renders strikethrough + monospace + hex-color spans", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(lines.locator(".scrollback-mirc-strikethrough", { hasText: strikeTag })).toHaveCount(1, { timeout: 10_000 });
