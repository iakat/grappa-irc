...
test('#319 landscape 5" — slim proportioned rails, center gets the majority, single-line topic', async ({ page, }) => {
    const vjt = specUser();
    if (!CHANNEL)
    ...
    if (!sidebarBox || !membersBox || !mainBox)
        throw new Error("shell panes have no bounding box");
    ...
    // widths clear.
    expect(sidebarBox.width, `#319 — left rail ${sidebarBox.width}px must be slim (< 176px) in landscape-compact`).toBeLessThan(176);
    expect(membersBox.width, `#319 — right rail ${membersBox.width}px must be slim (< 176px) in landscape-compact`).toBeLessThan(176);
    ...
    expect(mainBox.width, `#319 — center pane ${mainBox.width}px must exceed half the 844px viewport`).toBeGreaterThan(844 / 2);
    ...
    expect(nameFits, "#319 — channel name must not be ellipsis-truncated in landscape-compact").toBe(true);
});
