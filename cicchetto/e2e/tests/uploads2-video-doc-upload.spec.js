...
test("uploads-2 — document: picker upload.txt → 📄 link in scrollback → GET serves bytes", async ({ page, }) => {
    const body = fixture("upload.txt");
    await openChannel(page);
    const { slug } = await uploadViaPicker(page, { name: "upload.txt", mimeType: "text/plain", buffer: body }, { postTimeout: 10_000 });
...
test("uploads-2 — video (chromium): picker tiny.mp4 → transcode-or-fallback → 🎬 link in scrollback", async ({ page, }) => {
    await openChannel(page);
    ...
    () => !("VideoEncoder" in globalThis)), "no WebCodecs VideoEncoder in this browser build");
    const { slug } = await uploadViaPicker(page, { name: "tiny.mp4", mimeType: "video/mp4", buffer: fixture("tiny.mp4") }, 
