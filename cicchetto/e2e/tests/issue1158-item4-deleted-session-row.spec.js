...
test("#1158 item 4 a deleted visitor's session survives on the ended-sessions page", async ({ page, }) => {
    ...
    try {
        ...
        await expect
            ...
            .toBeGreaterThan(0);
        const [logged] = (await listSessionLogSessions(admin.token)).filter((e) => e.session_id.startsWith(sessionIdPrefix));
        if (logged === undefined)
            throw new Error("precondition vanished between poll and read");
