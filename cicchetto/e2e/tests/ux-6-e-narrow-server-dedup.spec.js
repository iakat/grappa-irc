...
test("@webkit @touch UX-6-E — narrow mode renders one network entry; no standalone 'Server' tab", async ({ page, }) => {
    const vjt = specUser();
...
test("@webkit @touch UX-6-E — clicking the network-header focuses the server window", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
...
test("@webkit @touch UX-6-E — network-header has a disconnect × sibling, mirroring sidebar", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    const section = page.locator(".bottom-bar-network", {
        has: page.locator(`.bottom-bar-network-header[data-network-slug="${NETWORK_SLUG}"]`),
    ...
    const disconnectBtn = section.locator(`.bottom-bar-network-header[data-network-slug="${NETWORK_SLUG}"] + .bottom-bar-close`);
