...
test("issue #211 phase 3 — fresh visitor lands JOINED with own nick + a live PRIVMSG (full read-cutover chain)", async ({ browser, }) => {
    ...
    try {
        ...
        await expect(page.locator('[data-testid="scrollback-line"][data-kind="notice"]').first()).toBeVisible({ timeout: 30_000 });
        ...
        await expect(page.locator(".sidebar-network-section li").filter({ hasText: channel })).toHaveCount(1, { timeout: 20_000 });
...
test("issue #211 phase 3 — a visitor CANNOT attach a non-visitor_enabled network (allowlist gate)", async () => {
    ...
    const networkId = (await createRes.json()).id;
    try {
