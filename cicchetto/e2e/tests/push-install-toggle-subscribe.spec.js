...
import { pushCatcherEndpoint, resetPushCatcher, resetPushSubscriptions, stubPushManager, } from "../fixtures/push";
import { AUTOJOIN_CHANNELS, NETWORK_SLUG } from "../fixtures/seedData";
import { expect, specNick, specUser, test } from "../fixtures/test";
const SUB_ID = "install-happy";
test("master toggle enables push: subscribe → POST /push/subscriptions → device list shows 1", async ({ page, context, }) => {
    const vjt = specUser();
    ...
    const subscribePostPromise = page.waitForResponse((resp) => resp.url().endsWith("/push/subscriptions") && resp.request().method() === "POST", { timeout: 5_000 });
    ...
    const created = (await subscribeResp.json());
