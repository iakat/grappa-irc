...
test("#913 — the safe-area inset shrinks the rail menu cap, pushing its top row clear", async ({ page, }) => {
    const vjt = specUser();
    ...
    expect(overflowsBefore, "rail menu must overflow for the cap to be the binding constraint").toBe(true);
    ...
    if (boxBefore && boxAfter) {
        ...
        expect(boxAfter.y - boxBefore.y, "the top row must descend by exactly the safe-area inset").toBeGreaterThan(STUB_INSET - 1.5);
        expect(boxAfter.y - boxBefore.y, "the top row must not descend by MORE than the inset (double-counted)").toBeLessThan(STUB_INSET + 1.5);
    }
});
