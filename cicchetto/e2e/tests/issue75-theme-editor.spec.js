...
async function setEditorColor(page, key, value) {
    await page.getByTestId(`theme-editor-color-${key}`).evaluate((el, v) => {
        el.value = v;
        el.dispatchEvent(new Event("input", { bubbles: true }));
    }, value);
}
async function openThemesGalleryDesktop(page) {
...
// row is the path now.
async function openThemesGalleryMobile(page) {
...
async function paintedTextContrast(page, testId) {
    ...
    return page.evaluate(async (b64) => {
        ...
        const far = (k) => Math.max(...rgb(k).map((c, i) => Math.abs(c - rgb(fill)[i]))) > 40;
...
test.describe("#75 — theme editor (producer path)", () => {
    ...
    test("self-hosted font applies live from same-origin /fonts (no CDN)", async ({ page }) => {
        ...
        await expect
            .poll(() => page.evaluate(() => document.documentElement.style.getPropertyValue("--font-mono")), { timeout: 5_000 })
        ...
        expect(requests.some((u) => /fonts\.googleapis\.com|fonts\.gstatic\.com|fonts\.google/i.test(u))).toBe(false);
        await page.getByTestId("theme-editor-cancel-btn").click();
    });
    test("background upload: wallpaper layer applies live + persists across reload", async ({ page, }) => {
        await loginAs(page, specUser());
        ...
        const bgImage = await page.evaluate(() => document.documentElement.style.getPropertyValue("--theme-bg-image").trim());
        ...
        await expect
            .poll(() => page.evaluate(() => document.documentElement.style.getPropertyValue("--theme-bg-opacity").trim()), { timeout: 5_000 })
            .toBe("0.55");
        ...
        await expect(page.locator("html.theme-has-bg")).toHaveCount(1, { timeout: 15_000 });
        await expect
            .poll(() => page.evaluate(() => document.documentElement.style.getPropertyValue("--theme-bg-opacity").trim()), { timeout: 10_000 })
            .toBe("0.55");
    });
});
