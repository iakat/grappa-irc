...
test.describe("#75 — themes gallery consumer flow", () => {
    test("@webkit @touch 🎨 launcher opens the themes sub-page with the built-in gallery", async ({ page, }) => {
        const vjt = specUser();
        ...
        await expect(cards.first().locator("[data-testid^='theme-swatch-'] .theme-chip").first()).toBeVisible();
    ...
    test("@webkit @touch tapping a card flips --bg live and persists across reload via the server", async ({ page, }) => {
        const vjt = specUser();
        await loginAs(page, vjt);
        await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
        await expect(sidebarWindow(page, NETWORK_SLUG, CHANNEL)).toBeVisible();
