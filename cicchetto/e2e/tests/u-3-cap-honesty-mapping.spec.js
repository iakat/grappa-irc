...
import { ADMIN_IDENTIFIER, ADMIN_PASSWORD, AUTOJOIN_CHANNELS, getSeededAdmin, M9B_USER, NETWORK_SLUG, } from "../fixtures/seedData";
...
async function connectWithClientId(token, slug, clientId) {
...
async function loginWithClientId(identifier, password, clientId) {
...
test("U-3 — per-IP cap saturation → 503 too_many_sessions", async () => {
    ...
    const body = (await m9bConnect.json());
