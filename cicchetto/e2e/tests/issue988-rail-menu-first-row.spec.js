...
import { closeSettings, loginAs, openRailMenu, openSettingsSection, selectChannel, } from "../fixtures/cicchettoPage";
...
async function measureMenu(page) {
    return page.evaluate(() => {
        ...
        if (menu === null)
            throw new Error("#988 — the rail actions menu is not mounted");
        const rows = menu.querySelectorAll(".rail-action");
        const first = rows[0];
        if (first === undefined)
            throw new Error("#988 — the menu has no rows");
        const menuRect = menu.getBoundingClientRect();
        const rowRect = first.getBoundingClientRect();
        const menuStyle = getComputedStyle(menu);
        const hit = document.elementFromPoint(rowRect.x + rowRect.width / 2, rowRect.y + rowRect.height / 2);
        return {
...
function assertFirstRowVisible(g, label) {
    ...
    expect(g.firstRowTop, `${label} — the first row starts ${g.firstRowTop}px from the viewport top, i.e. off-screen`).toBeGreaterThanOrEqual(-0.5);
    ...
    expect(Math.abs(g.firstRowTop - (g.menuTop + chromeAbove)), `${label} — the first row is ${g.firstRowTop - g.menuTop}px below the menu's top, but the menu's own chrome above it is only ${chromeAbove}px (border ${g.borderTopWidth} + padding ${g.paddingTop}); the remainder is unexplained`).toBeLessThanOrEqual(0.5);
    ...
    expect(g.firstRowBottom, `${label} — the first row is not WHOLLY visible inside the scroll port`).toBeLessThanOrEqual(g.menuTop + g.borderTopWidth + g.clientHeight + 1);
    // Visible is not the same as reachable: `home` must also be the element
    // under the finger, not something painted over it.
    expect(g.firstRowIsHitTarget, `${label} — the first row is covered by another element`).toBe(true);
...
async function reportReadings(testInfo, label, readings) {
...
test("@webkit @touch #988 — the actions menu opens with `home` visible at every viewport height", async ({ page, }, testInfo) => {
    const vjt = specUser();
    ...
    // not the assertion.
    expect(readings.some(({ g }) => g.scrollHeight > g.clientHeight + 1), "#988 — no viewport in the set made the menu overflow; the capped case was never exercised").toBe(true);
});
test("@webkit @touch #988 — `home` stays visible at XXL text with the viewport at its shortest", async ({ page, }, testInfo) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
    try {
        ...
        expect(await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue("--font-size").trim())).toBe("20px");
