...
import { accreteNetwork, clearMutedConversations, GRAPPA_BASE_URL, muteKey, partChannel, patchNetworkConnectionState, } from "../fixtures/grappaApi";
import { IrcPeer } from "../fixtures/ircClient";
import { assertNoPushDelivery, awaitPushDelivery, enablePushFromSettings, pushCatcherEndpoint, resetPushCatcher, resetPushSubscriptions, setPageVisibility, stubPushManager, } from "../fixtures/push";
import { getSeededMute1038User, MUTE1038_HOST_A, MUTE1038_HOST_B, MUTE1038_NETWORK_A, MUTE1038_NETWORK_B, MUTE1038_USER, } from "../fixtures/seedData";
...
async function waitForConnected(token, slug) {
    for (let i = 0; i < 60; i++) {
        const rows = await getNetworks(token);
        if (rows.find((r) => r.slug === slug)?.connection_state === "connected")
            return;
...
test("a mute made on one network leaves the same channel on the other still pushing", async ({ page, context, }) => {
    ...
    try {
        ...
        await expect(page.locator(`[data-testid="pref-muted-net-${keyA}"]`)).toHaveText(MUTE1038_NETWORK_A);
