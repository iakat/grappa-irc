...
import { composeSend, loginAs, scrollbackLine, selectChannel, waitForDmListenerReady, waitForQueryWindowReady, } from "../fixtures/cicchettoPage";
...
test("query window follows a peer NICK change — relabels, keeps history, routes with no 401", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(scrollbackLine(page, "join", OLD_NICK).filter({ hasText: CHANNEL }).first()).toBeVisible({ timeout: 10_000 });
        ...
        await expect(page.locator('[data-testid="scrollback-line"]', { hasText: OWN_BODY })).toBeVisible({ timeout: 5_000 });
        await waitForDmListenerReady(page, NETWORK_SLUG);
        peer.privmsg(await specLiveNick(), REPLY_BODY);
        await expect(page.locator('[data-testid="scrollback-line"]', { hasText: REPLY_BODY })).toBeVisible({ timeout: 5_000 });
        ...
        await expect(page.locator('[data-testid="scrollback-line"]', { hasText: OWN_BODY })).toBeVisible();
        await expect(page.locator('[data-testid="scrollback-line"]', { hasText: REPLY_BODY })).toBeVisible();
        ...
        await expect(page.locator('[data-testid="scrollback-line"]', { hasText: FOLLOWUP_BODY })).toBeVisible({ timeout: 5_000 });
    }
    finally {
