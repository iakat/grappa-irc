...
test("#159 — tab RE-SELECT after a socket-stays-open gap re-fetches the missed row", async ({ page, }) => {
    const vjt = specUser();
...
test("#159 — tab RE-FOREGROUND (hidden→visible) after a socket-stays-open gap re-fetches the missed row", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
