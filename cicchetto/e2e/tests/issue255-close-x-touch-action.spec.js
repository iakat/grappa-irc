...
async function closeXTouchAction(page, slug, channel) {
...
test("#255 — desktop .sidebar-close aligns touch-action to the sidebar's pan-y axis", async ({ page, }) => {
    const vjt = specUser();
...
test("#255 @webkit @touch — mobile .bottom-bar-close aligns touch-action to the bottom-bar's pan-x axis", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
