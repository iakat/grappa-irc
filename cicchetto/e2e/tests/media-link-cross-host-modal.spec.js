...
async function serveForeignMedia(page) {
...
async function foreignLink(page, url) {
...
test("cross-host https image link opens the media viewer, href unchanged, bytes CSP-admitted (#1240)", async ({ page, }) => {
    ...
    expect(csp, "server-served CSP must widen img-src for cross-host images").toContain("img-src 'self' data: blob: https:");
...
test("cross-host https video link opens the media viewer, href unchanged, metadata decoded (#1240)", async ({ page, }) => {
    await serveForeignMedia(page);
    await openChannel(page);
    ...
    await expect
        .poll(async () => video.evaluate((el) => el.videoWidth), {
        timeout: 15_000,
    })
        .toBeGreaterThan(0);
});
