...
test("query rail shows heading + auto-fetched WHOIS card, follows NICK, coexists with /whois", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(ctx.locator(".rail-query-heading")).toHaveText(new RegExp(`private conversation with ${PEER}`, "i"));
        ...
        await expect(ctx.locator(".rail-query-heading")).toHaveText(new RegExp(`private conversation with ${PEER_RENAMED}`, "i"), { timeout: 5_000 });
