...
async function snapshotStrip(page, scrollTo) {
    return page.evaluate((mode) => {
        const bar = document.querySelector(".bottom-bar");
        if (!bar)
            throw new Error("snapshotStrip: no .bottom-bar in the DOM");
        bar.scrollLeft = mode === "max" ? bar.scrollWidth : mode;
        const barRect = bar.getBoundingClientRect();
        const headers = Array.from(bar.querySelectorAll(".bottom-bar-network-header")).map((h) => {
            const r = h.getBoundingClientRect();
            return { slug: h.getAttribute("data-network-slug") ?? "", left: r.left, right: r.right };
        });
        const groups = Array.from(bar.querySelectorAll(".bottom-bar-network")).map((g) => {
            const r = g.getBoundingClientRect();
            const slug = g.querySelector(".bottom-bar-network-header")?.getAttribute("data-network-slug") ?? "";
...
const isAtEdge = (snap, slug) => {
    const h = snap.headers.find((x) => x.slug === slug);
    return h !== undefined && Math.abs(h.left - snap.barLeft) <= EDGE_EPS;
};
const edgeCount = (snap) => snap.headers.filter((h) => Math.abs(h.left - snap.barLeft) <= EDGE_EPS).length;
...
async function waitForNetworkState(token, slug, state, attempts = 60) {
    for (let i = 0; i < attempts; i++) {
        ...
        if (rows.find((r) => r.slug === slug)?.connection_state === state)
            return;
...
async function accreteNetwork(token, slug) {
...
async function joinChannelWhenReady(token, slug, channel) {
    let last = "";
    for (let i = 0; i < 60; i++) {
...
test("@webkit @touch #260 — the network header carries the sticky-left CSS contract", async ({ page, }) => {
...
test("@webkit @touch #260 — the sticky header pins under scroll and the next network displaces it", async ({ page, }) => {
    ...
    try {
        ...
        const channelsFor = (net) => [0, 1, 2, 3].map((i) => `#s260-${net}-channel-longname-${i}-${stamp}`);
        const anchorChans = channelsFor("a");
        const secondChans = channelsFor("b");
        for (const c of anchorChans)
            await joinChannelWhenReady(visitor.token, ANCHOR, c);
        for (const c of secondChans)
            await joinChannelWhenReady(visitor.token, SECOND, c);
        ...
        await expect
            ...
            if (s.headers.length !== 2)
                return `headers=${s.headers.length}`;
        ...
        if (outgoing)
            expect(outgoing.right).toBeLessThanOrEqual(atEnd.barLeft + EDGE_EPS);
    }
    finally {
