...
test("subprotocol path — cic connects with the token OFF the URL, members seed", async ({ page, }) => {
    const vjt = specUser();
...
test("legacy query-string path — a raw ?token= WS handshake is now rejected (#202 dropped the fallback)", async ({ page, }) => {
    const vjt = specUser();
