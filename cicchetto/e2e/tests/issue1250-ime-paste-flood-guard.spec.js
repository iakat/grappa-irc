...
import { composeTextarea, confirmModal, confirmModalBody, confirmModalCancel, confirmModalYes, loginAs, selectChannel, } from "../fixtures/cicchettoPage";
...
async function imePaste(page, text) {
...
test("#1250 — an over-limit IME paste is hard-closed: dialog, and the block never lands", async ({ page, }) => {
...
async function realPasteOrderJourney(page, testInfo) {
    await openCompose(page);
    const ta = composeTextarea(page);
    ...
    await ta.evaluate((el) => {
        ...
        for (const type of ["paste", "beforeinput"]) {
            el.addEventListener(type, (e) => {
                w.__pasteSeq?.push(type === "beforeinput" ? `beforeinput:${e.inputType}` : "paste");
            }, { capture: true });
        }
    });
    ...
    await expect(ta).toHaveValue("");
    const seq = await page.evaluate(() => window.__pasteSeq ?? []);
    ...
    expect(seq.filter((s) => s === "paste" || s === "beforeinput:insertFromPaste"), `a cancelled paste must be the ONLY report the guard sees — observed: ${seq.join(" → ")}`).toEqual(["paste"]);
}
test("#1250 — a REAL paste is reported once to the guard: one dialog, and the measured order", async ({ page, }, testInfo) => {
...
test("#1250 — a REAL paste is reported once to the guard: one dialog, and the measured order @webkit @touch", async ({ page, }, testInfo) => {
    await realPasteOrderJourney(page, testInfo);
});
