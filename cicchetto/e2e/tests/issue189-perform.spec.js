...
const clearPerform = (token) => fetch(`${GRAPPA_BASE_URL}/networks/${NETWORK_SLUG}/perform`, {
    method: "PUT",
    headers: { authorization: `Bearer ${token}`, "content-type": "application/json" },
    body: JSON.stringify({ perform_list: "", oper_pass: "" }),
}).catch(() => { });
test("#189 — perform editor: nav row, save list + oper pass, round-trips against server", async ({ page, }) => {
    const vjt = specUser();
    try {
        ...
        await page.getByTestId("perform-settings-entry").click();
        await expect(page.getByTestId(`perform-list-${NETWORK_SLUG}`)).toHaveValue("MODE mynick +x\nWHOIS mynick");
