...
import { assertMessagePersisted, GRAPPA_BASE_URL, mintVisitor, reapVisitors, } from "../fixtures/grappaApi";
...
async function assertAlive(token, label) {
...
async function setNetworkNick(token, slug, nick) {
...
async function waitForOwnNickInMembers(token, slug, channel, ownNick) {
    for (let i = 0; i < 40; i++) {
...
test("issue #211 phase 7 — one visitor on TWO networks survives a real cic WS drop and reappears LIVE on BOTH", async ({ browser, }) => {
    ...
    try {
        ...
        await expect(page.locator(".shell-members .members-pane .member-name", { hasText: anchorNick })).toBeVisible({ timeout: 15_000 });
        await selectChannel(page, SECOND, secondChannel, { ownNick: secondNick });
        await expect(page.locator(".shell-members .members-pane .member-name", { hasText: secondNick })).toBeVisible({ timeout: 15_000 });
        ...
        await expect(page.locator('[data-testid="scrollback-line"][data-kind="privmsg"]', {
        ...
        await expect(page.locator(".shell-members .members-pane .member-name", { hasText: anchorNick })).toBeVisible({ timeout: 20_000 });
        await selectChannel(page, SECOND, secondChannel, { awaitWsReady: false });
        await expect(page.locator(".shell-members .members-pane .member-name", { hasText: secondNick })).toBeVisible({ timeout: 20_000 });
