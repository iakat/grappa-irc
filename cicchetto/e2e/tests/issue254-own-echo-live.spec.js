...
async function installSendPostReadinessProbe(page, readyKey) {
...
async function assertSubscribeBeforeSend(page, peerNick, body) {
    ...
    await page.waitForFunction(() => window.__i254_readyAtPost != null, undefined, { timeout: 10_000 });
    // The deterministic contract: the topic was joined + ACKed BEFORE the POST.
    const readyAtPost = await page.evaluate(() => window.__i254_readyAtPost);
...
test("#254 — /msg to a fresh query window subscribes BEFORE the send POST (own echo has a live listener)", async ({ page, }) => {
    const vjt = specUser();
...
test("@webkit @touch #254 — /msg to a fresh query window subscribes BEFORE the send POST on iOS", async ({ page, }) => {
    const vjt = specUser();
...
async function dropSocketAndHold(page) {
...
async function assertOwnChannelMsgRendersAfterWake(page, peerNick) {
    const peer = await IrcPeer.connect({ nick: peerNick });
    try {
...
test("#254 — own channel message renders live after an iOS-style suspend/resume (no reload)", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
    await assertOwnChannelMsgRendersAfterWake(page, `i254c-${crypto.randomUUID().slice(0, 8)}`);
});
test("@webkit @touch #254 — own channel message renders live after iOS suspend/resume", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
