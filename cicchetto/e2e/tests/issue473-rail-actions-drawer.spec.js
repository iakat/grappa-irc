...
import { closeMembersDrawer, expandArchiveGroup, loginAs, openArchive, openMembersDrawer, openRailMenu, selectChannel, sidebarWindow, } from "../fixtures/cicchettoPage";
...
test.describe("#473 — RailActions drawer + grouped ArchiveModal", () => {
    test("desktop channel window — rail hosts every labelled button; admin absent for a non-admin", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("desktop home — archive is always-on (no selection) and opens the grouped modal", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("desktop — PART → grouped modal lazy-loads the row → clicking it revives + closes the modal", async ({ page, }) => {
        const vjt = specUser();
        await loginAs(page, vjt);
        await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
        await expect(sidebarWindow(page, NETWORK_SLUG, CHANNEL)).toBeVisible();
    ...
    test("@webkit @touch mobile — drawer hosts every labelled button; PART → rail archive → grouped modal row", async ({ page, }) => {
        const vjt = specUser();
        await loginAs(page, vjt);
        await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
        await expect(sidebarWindow(page, NETWORK_SLUG, CHANNEL)).toBeVisible();
