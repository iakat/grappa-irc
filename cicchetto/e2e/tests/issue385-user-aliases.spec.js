...
test("#385 — define via /alias, then the expanded command takes effect (/act → /me)", async ({ page, }) => {
    const vjt = specUser();
    try {
...
test("#385 — aliases settings sub-page: deep-link, add round-trip, 422 inline, × remove", async ({ page, }) => {
    const vjt = specUser();
    try {
        await clearAliases(vjt.token);
        await loginAs(page, vjt);
