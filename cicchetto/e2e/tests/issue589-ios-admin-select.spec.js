...
test("@webkit iOS — admin pane content re-enables user-select under the is-ios global kill; controls stay unselectable", async ({ page, }) => {
    ...
    try {
        ...
        const styles = await page.evaluate((rowKey) => {
            ...
            const button = document.querySelector(`[data-testid="admin-session-disconnect-${rowKey}"]`) ??
