...
import { bootVisitorContext, closeSettings, expectShellReady, openRailMenu, openSettingsDrawer, } from "../fixtures/cicchettoPage";
...
async function createThrowawayUser(adminToken, name, password) {
...
async function deleteUserBestEffort(adminToken, id) {
    try {
...
async function loginRejected(identifier, password) {
...
test.describe("issue #157 — delete account", () => {
    test("registered non-admin user wipes their account via the drawer; the login stops working", async ({ page, }) => {
        ...
        try {
            ...
            await page.addInitScript(([t, subjectJson]) => {
                localStorage.setItem("grappa-token", t);
                localStorage.setItem("grappa-subject", subjectJson);
                localStorage.setItem("cic.installChoice", "browser");
            }, [token, JSON.stringify(subject)]);
            await page.goto("/");
    ...
    test("#987 — the delete-account dialog is centred and sized to its content", async ({ page }) => {
        ...
        try {
            ...
            expect(box.y + box.height, "scrim must be visible below the dialog").toBeLessThan(scrim.y + scrim.height);
            ...
            expect(box.height, "a short dialog must not claim most of the screen").toBeLessThan(scrim.height * 0.75);
