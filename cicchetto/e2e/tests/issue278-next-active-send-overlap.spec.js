...
test("#278 @webkit @touch — with the keyboard open the next-active circle does not overlap the send button and stays reachable", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect
            ...
            message: "keyboard-open: next-active circle must NOT overlap the send button (#264's 4rem focus-lift drops it onto send)",
            timeout: 5_000,
        })
            .toBe(0);
        ...
        if (na && sb) {
            expect(overlapArea(na, sb), `keyboard-open overlap must be 0 — next-active ${fmt(na)} vs send ${fmt(sb)}`).toBe(0);
            ...
            if (vp) {
                ...
                expect(na.y + na.height, `next-active bottom ${(na.y + na.height).toFixed(0)} must be ≤ viewport height ${vp.height}`).toBeLessThanOrEqual(vp.height + 0.5);
                expect(na.x + na.width, `next-active right ${(na.x + na.width).toFixed(0)} must be ≤ viewport width ${vp.width}`).toBeLessThanOrEqual(vp.width + 0.5);
            }
        }
    }
    finally {
