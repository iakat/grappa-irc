...
test("#975 — /mode on a parted channel reports unknown, not the modes cached while joined", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(page.locator(".sidebar-network-section li").filter({ hasText: channel })).toHaveCount(0, { timeout: 10_000 });
