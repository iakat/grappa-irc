...
import { bootVisitorContext, composeSend, composeTextarea, expectShellReady, scrollbackLine, selectChannel, waitForUserTopicReady, } from "../fixtures/cicchettoPage";
import { mintVisitor, reapVisitors } from "../fixtures/grappaApi";
import { getSeededAdmin } from "../fixtures/seedData";
import { expect, test } from "../fixtures/test";
test("issue #154(1) — a rejected ops verb surfaces an inline compose error (no silent green ✓)", async ({ browser, }) => {
...
test("issue #154(2) — an own-nick /umode renders a 'sets user mode' row in $server", async ({ browser, }) => {
    const admin = getSeededAdmin();
    const stamp = Date.now();
