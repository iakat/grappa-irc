...
// down the peer.
import { composeSend, confirmModal, confirmModalYes, expectShellReady, loginAs, selectChannel, sidebarCloseButton, sidebarWindow, } from "../fixtures/cicchettoPage";
...
function vjtUserId() {
    return JSON.parse(specUser().subjectJson).id;
}
test("#511 — a dismissed JOIN-FAILED autojoin row does NOT return after a reload, while a non-dismissed sibling does", async ({ page, }) => {
