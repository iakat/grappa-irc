...
test.describe("#161/#693 — a gap larger than one page opens at the tail, not the cursor", () => {
    ...
    test("unread > 200: the newest row is rendered on arrival, with an uncapped jump-back count and no divider", async ({ page, }) => {
        if (!CHANNEL)
        ...
        if (!lastReadRow)
            throw new Error("#161 spec: seeded #spec-wN rows missing cursor index");
        ...
        if (!newestPrivmsg)
            throw new Error("#161 spec: no seeded privmsg rows found");
        ...
        await expect(page.locator(`[data-testid="scrollback-line"][data-msg-id="${newestPrivmsg.id}"]`)).toBeVisible();
