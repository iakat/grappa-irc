...
test("@webkit @touch #985 — a mobile query window spends no band on the ☰, and the float stays legible and reachable", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        expect(paneBox.y - mainBox.y, `#985 — scrollback must start at the pane top; it starts ${paneBox.y - mainBox.y}px below it`).toBeLessThanOrEqual(1);
        ...
        expect(alpha, `#985 — the floated opener's backing must be fully opaque; computed ${bg}`).toBe(1);
