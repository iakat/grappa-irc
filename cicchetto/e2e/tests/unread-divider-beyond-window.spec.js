...
test.describe("#156 unread divider with unread beyond the fetch window", () => {
    test("anchors the divider between last-read and first-unread with context above", async ({ page, }) => {
        if (!CHANNEL)
        ...
        const expectedUnread = rows.filter((r) => r.id > lastReadRow.id &&
            !(OWN_PRESENCE_KINDS.has(r.kind) && r.sender.toLowerCase() === specNick().toLowerCase())).length;
        ...
        const scrollbackLine = (id) => page.locator(`[data-testid="scrollback-line"][data-msg-id="${id}"]`);
        ...
        await expect(scrollbackLine(lastReadRow.id).locator("xpath=following-sibling::*[1]")).toHaveAttribute("data-testid", "unread-marker");
        // …and the first-unread row is the row IMMEDIATELY AFTER the marker.
        await expect(marker.locator("xpath=following-sibling::*[1]")).toHaveAttribute("data-msg-id", String(firstUnreadRow.id));
        // ── Count is the TRUE unread, not the window size ───────────────────
        await expect(page.locator(".scrollback-unread-marker-label")).toHaveText(`${expectedUnread} unread messages`);
    });
});
