...
async function assertMotdPersisted(token) {
    ...
    while (Date.now() < deadline) {
        ...
        if (res.ok) {
            ...
            const matched = messages.find((m) => m.kind === "notice" && LEAF_SENDER_PATTERN.test(m.sender));
            if (matched)
                return;
    ...
    throw new Error(`assertMotdPersisted: no kind=notice from leaf[46].azzurra.chat in $server scrollback within 5s; last seen: ${JSON.stringify(lastSeen)}`);
}
