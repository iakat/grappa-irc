...
test.describe("#204 foolproof login", () => {
    ...
    test("Advanced toggle reveals realname + ident and sits between nick and Connect", async ({ page, }) => {
        ...
        const order = await page.evaluate(() => {
            ...
            if (!nick || !adv || !connect)
                return null;
            const nickBeforeAdv = Boolean(nick.compareDocumentPosition(adv) & Node.DOCUMENT_POSITION_FOLLOWING);
            const advBeforeConnect = Boolean(adv.compareDocumentPosition(connect) & Node.DOCUMENT_POSITION_FOLLOWING);
            return { nickBeforeAdv, advBeforeConnect };
        });
        expect(order).toEqual({ nickBeforeAdv: true, advBeforeConnect: true });
    });
    test("on-submit: space→_ sanitization hits the wire + connecting spinner renders", async ({ page, }) => {
