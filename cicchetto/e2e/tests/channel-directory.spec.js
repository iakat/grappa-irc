...
test("channel-directory — browse, no /messages fetch (#81 guard), search filter, one-click join", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        expect(messagesRequests.filter((url) => url.includes(listMessagesPath)), "GET /messages must NOT fire when selecting kind=list — grappa-irc#81 guard").toHaveLength(0);
