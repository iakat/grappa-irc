...
import { accreteNetwork, assertMessagePersisted, partChannel, patchNetworkConnectionState, restoreReadCursorToTail, } from "../fixtures/grappaApi";
import { IrcPeer } from "../fixtures/ircClient";
import { getSeededI498User, I498_CHANNEL, I498_NETWORK_SLUG, I498_NICK, } from "../fixtures/seedData";
...
const joinedTopicKeys = (page) => page.evaluate(() => window.__cic_joinedTopicKeys?.() ??
    []);
...
async function expectOwnMember(page, nick) {
...
test("#1340 — after a /nick, own-PART still tears the per-channel subscription down", async ({ page, }) => {
    const user = getSeededI498User();
...
test("#498 — a mention of the LIVE (renamed) nick lifts the server badge on cold-load", async ({ page, }) => {
    const user = getSeededI498User();
    const runId = crypto.randomUUID().slice(0, 8);
