...
test("a one-hour snooze picked from the rail silences the channel and says how long it has left", async ({ page, context, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect(page.getByTestId(`pref-muted-until-${snoozedKey}`)).toHaveText(/^(1h \d+m|5\dm) left$/);
