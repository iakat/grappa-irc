...
async function aliasModalJourney(page) {
    const vjt = specUser();
    ...
    // resolve to.
    const { slug, url } = await uploadViaPicker(page, { name: "alias-viewer.png", mimeType: "image/png", buffer: Buffer.from(TINY_PNG_HEX, "hex") }, { postTimeout: 10_000 });
...
test("upload link on a server-advertised host alias opens the in-app viewer re-rooted to the page origin (#324)", async ({ page, }) => {
    await aliasModalJourney(page);
});
test("upload link on a server-advertised host alias opens the in-app viewer re-rooted to the page origin (#324) @webkit @touch", async ({ page, }) => {
    await aliasModalJourney(page);
});
