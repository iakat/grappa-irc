...
// per-run-unique).
let peerSeq = 0;
test("#327 @webkit @touch — tapping next-active scrolls a far-off-screen unread tab fully into the bottom bar", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        await expect
            ...
            message: "TARGET tab must scroll fully into the bottom bar's visible bounds after the next-active jump (#327 stale-geometry regression)",
            timeout: 5_000,
        })
            .toBe(true);
    }
