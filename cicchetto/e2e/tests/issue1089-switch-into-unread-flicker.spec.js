...
import { loginAs, scrollbackLines, selectChannel, sidebarWindow, waitForScrollbackRefreshed, } from "../fixtures/cicchettoPage";
...
async function installFlickerProbe(page, windowMs) {
    await page.evaluate((windowMsArg) => {
        ...
        const target = document.querySelector('[data-testid="scrollback"]');
        ...
        if (target && desc?.get && desc?.set) {
            Object.defineProperty(target, "scrollTop", {
                ...
                set(v) {
                    w.__i1089writes.push({
                        ...
                        before: Math.round(desc.get?.call(target)),
                    });
            ...
            Element.prototype.scrollIntoView = function (arg) {
                if (this === target || target.contains(this)) {
                    w.__i1089writes.push({
                        t: Math.round(performance.now() - t0),
                        kind: "scrollIntoView",
                        detail: `${this.dataset?.testid ?? this.className} ${JSON.stringify(arg ?? null)}`,
                        before: Math.round(desc.get?.call(target)),
                    });
                }
        ...
        const loop = () => {
            ...
            if (el) {
                const m = el.querySelector('[data-testid="unread-marker"]');
...
async function waitForProbeBaseline(page) {
    await page.waitForFunction(() => (window.__i1089frames?.length ?? 0) > 0, undefined, { timeout: 10_000 });
}
...
test.describe("issue #1089 — switching into an unread window must not flicker", () => {
    ...
    test("SWITCH into channel-with-unreads: the divider never leaves the viewport on a visible frame", async ({ page, }) => {
        const vjt = specUser();
        if (!CHANNEL)
    ...
    test("live append while parked on the divider: the re-assert must not paint a displaced frame", async ({ page, }) => {
        const vjt = specUser();
        if (!CHANNEL)
            throw new Error("AUTOJOIN_CHANNELS empty");
        await seedMidPageCursor(vjt.token, CHANNEL);
...
async function seedMidPageCursor(token, channel) {
...
async function warmUpChannel(page, channel) {
...
async function parkOnServerWindow(page) {
...
async function delayMessageFetches(page, ms) {
...
async function assertNoVisibleJump(page, tag) {
    const frames = await page.evaluate(() => window.__i1089frames);
    const writes = await page.evaluate(() => window.__i1089writes ?? []);
    ...
    expect(settled.marker).toBeGreaterThanOrEqual(-MARKER_TOP_TOLERANCE_PX);
    expect(settled.marker).toBeLessThan(settled.client);
    ...
    const offscreen = frames.filter((f) => f.vis === "visible" &&
        f.marker !== null &&
        (f.marker < -MARKER_TOP_TOLERANCE_PX || f.marker >= f.client));
    expect(offscreen, `visible frame(s) with the divider off-screen — the operator saw the jump: ${JSON.stringify(offscreen)}`).toEqual([]);
    return frames;
}
