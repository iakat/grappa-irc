...
import { closeSettings, loginAs, openRailMenu, selectChannel, sidebarWindow, } from "../fixtures/cicchettoPage";
...
test("@webkit @touch ux-5-bm mobile-channel — topic-bar hosts hamburger only; drawer hosts settings+archive launchers; mutex enforced", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
    await selectChannel(page, NETWORK_SLUG, CHANNEL, { ownNick: specNick() });
...
test("@webkit @touch ux-5-bm mobile-non-channel — home keeps its own ☰ door to the rail", async ({ page, }) => {
    const vjt = specUser();
    await loginAs(page, vjt);
