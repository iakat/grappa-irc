...
test("admin adds a server to a network via the disclosure", async ({ page }) => {
    ...
    try {
        ...
        await expect(page.locator(`[data-testid^='admin-network-server-row-${slug}-']`)).toHaveCount(1, { timeout: 5_000 });
    }
    finally {
        if (networkId !== null)
...
test("admin sees a per-network source rejected via the add-server form (non-local)", async ({ page, }) => {
    const admin = getSeededAdmin();
...
test("admin sees an inline source edit rejected on an existing server (non-local)", async ({ page, }) => {
    const admin = getSeededAdmin();
...
test("admin deletes a server via inline-confirm", async ({ page }) => {
    ...
    try {
        ...
        await delBtn.click();
        // Row gone.
        await expect(page.locator(`[data-testid='admin-network-server-row-${slug}-${serverId}']`)).toHaveCount(0, { timeout: 5_000 });
