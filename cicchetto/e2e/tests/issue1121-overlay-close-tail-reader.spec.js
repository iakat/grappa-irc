...
test.describe("#1121 — closing an overlay over a tail reader must not strand them", () => {
    ...
    test("closing the media viewer over a tail reader restores the button and keeps following (desktop)", async ({ page, }) => {
        test.slow();
        if (!CHANNEL)
        ...
        try {
            ...
            await expect(scrollbackLine(page, "join", new RegExp(`${PEER_NICK} \\[~?[^\\]]+\\] has joined`))).toBeAttached({ timeout: 15_000 });
