...
import { fetchAllMessagesAsc, restoreReadCursorToTail, setReadCursorToId, } from "../fixtures/grappaApi";
...
test.describe("#887 focused-window unread badge", () => {
    test("survives focus, holds while scrolled up, and falls as the operator reads", async ({ page, }) => {
        if (!CHANNEL)
            throw new Error("AUTOJOIN_CHANNELS empty");
        const vjt = specUser();
        ...
        if (!lastRead)
            throw new Error("#887 spec: seeded #spec-wN rows missing expected index");
