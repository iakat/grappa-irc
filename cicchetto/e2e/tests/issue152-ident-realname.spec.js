...
import { composeSend, expectShellReady, openSettingsSection, selectChannel, waitForUserTopicReady, } from "../fixtures/cicchettoPage";
import { reapVisitors } from "../fixtures/grappaApi";
import { IrcPeer } from "../fixtures/ircClient";
import { getSeededAdmin } from "../fixtures/seedData";
import { expect, test } from "../fixtures/test";
test("issue #152 — login-Advanced ident + settings live-apply reach upstream", async ({ browser, }) => {
    ...
    try {
        ...
        const identWitness1 = peer.waitForLine(new RegExp(`:${visitorNick}!~${loginIdent}@\\S+ PRIVMSG ${channel} :.*${marker1}`, "i"), `visitor login-ident ~${loginIdent}`);
        ...
        const identWitness2 = peer.waitForLine(new RegExp(`:${visitorNick}!~${newIdent}@\\S+ PRIVMSG ${channel} :.*${marker2}`, "i"), `visitor reconnected-ident ~${newIdent}`, 30_000);
        ...
        if (peer)
            await peer.disconnect("issue152 done").catch(() => { });
        await ctx.close();
