...
import { bootVisitorContext, composeSend, expectShellReady, scrollbackLine, selectChannel, sidebarWindow, } from "../fixtures/cicchettoPage";
...
test("issue #184 — /stats reply renders in $server, never a query window named after the letter", async ({ browser, }) => {
    ...
    try {
        ...
        const url = `${GRAPPA_BASE_URL}/networks/${encodeURIComponent(visitor.network_slug)}/channels/${encodeURIComponent(STATS_LETTER)}/messages`;
