...
// fold).
import { composeTextarea, loginAs, scrollbackDistanceFromBottom, scrollbackLines, selectChannel, waitForScrollbackRefreshed, } from "../fixtures/cicchettoPage";
...
async function installFailingSendPost(page, hold) {
    await page.addInitScript((holdUntilReleased) => {
        let release = () => { };
...
async function arriveParkedOnMarker(page, channel) {
    const vjt = specUser();
...
async function sendAndAwaitEcho(page, marker) {
...
test.describe("issue #580 — own send snaps to the bottom independent of the POST", () => {
    ...
    test("the send-failure banner does not push the sent row under the fold (case 2, #778)", async ({ page, }) => {
        if (!CHANNEL)
            throw new Error("AUTOJOIN_CHANNELS empty");
