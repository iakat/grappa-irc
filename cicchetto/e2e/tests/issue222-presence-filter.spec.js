...
test("#222 — per-channel toggle hides join/part rows, persists across reload, PRIVMSG stays", async ({ page, }) => {
    const vjt = specUser();
    ...
    try {
        ...
        if (toggleBox === null)
            throw new Error("presence toggle has no bounding box");
        expect(Math.round(toggleBox.height), `#305 — presence toggle tap target ${toggleBox.height}px must meet the 48px HIG floor`).toBeGreaterThanOrEqual(48);
        const toggleGlyphPx = await toggle.evaluate((el) => parseFloat(getComputedStyle(el).fontSize));
        expect(toggleGlyphPx, `#305 — presence toggle glyph ${toggleGlyphPx}px must be enlarged from the base 14px`).toBeGreaterThanOrEqual(18);
