...
test.describe("UX-6-C / #473 — admin launcher in the rail actions drawer", () => {
    ...
    test("@webkit @touch admin on mobile — rail actions drawer hosts admin button; tap opens AdminPane", async ({ page, }) => {
        const admin = getSeededAdmin();
    ...
    test("@webkit @touch non-admin on mobile — rail actions drawer hides the admin button", async ({ page, }) => {
    ...
    test("desktop admin — members rail hosts the RailActions admin button (retired mobile footer is gone)", async ({ page, }) => {
        const admin = getSeededAdmin();
        await setAdminFlag(admin.token, vjtUserId, true);
        const vjt = specUser();
        await loginAs(page, vjt);
        ...
        await openRailMenu(page);
        await expect(page.locator(".rail-actions-menu [data-testid='mobile-panel-admin']")).toHaveCount(1);
