...
import { confirmModal, loginAs, selectChannel, sidebarCloseButton, sidebarWindow, } from "../fixtures/cicchettoPage";
import { AUTOJOIN_CHANNELS, NETWORK_SLUG } from "../fixtures/seedData";
import { expect, specNick, specUser, test } from "../fixtures/test";
const CHANNEL = AUTOJOIN_CHANNELS[0];
test.setTimeout(60_000);
test("#407 the -full scrim spans to the viewport bottom, and a click there dismisses", async ({ page, }) => {
    const vjt = specUser();
    ...
    if (viewport === null)
        throw new Error("no viewport size — headless config changed");
    const box = await scrim.boundingBox();
    if (box === null)
        throw new Error("the scrim has no box: it is not rendered");
    ...
    expect(Math.round(box.y + box.height), "the scrim must reach the viewport bottom").toBe(viewport.height);
