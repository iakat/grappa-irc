...
test.describe("#263 topic editing inside the modal", () => {
    test("op edits the topic from the modal: read-only → ✏️ → cancel-stays-open → esc-cancel → multi-line save flattens + closes", async ({ page, }) => {
        const vjt = specUser();
    ...
    test("save on a rejected topic preserves the draft and keeps the modal open (S21)", async ({ page, }) => {
