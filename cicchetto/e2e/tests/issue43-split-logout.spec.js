...
test("issue #43 — registered user sees detach + quit in the rail, not a bare 'log out'", async ({ page, }) => {
...
test("issue #43/#986 — quit opens a confirm modal that explains, and Cancel changes nothing", async ({ page, }) => {
    await loginAs(page, specUser());
    await openRailMenu(page);
