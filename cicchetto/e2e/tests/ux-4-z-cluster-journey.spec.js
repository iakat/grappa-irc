...
import { closeMembersDrawer, closeSettings, composeSend, loginAs, openAdminConsole, openSettingsMobile, selectChannel, sidebarWindow, } from "../fixtures/cicchettoPage";
...
const CLASSES = [
    ...
    {
        name: "visitor",
        skipReason: "visitor mint unblocked (e2e-revive-skips); driving this arm is a follow-up — HomePane + windowClose unit-cover the visitor branch",
    },
    {
        name: "nickserv",
        skipReason: "no nickserv-identified user seeded in e2e harness (vjt bind uses --auth password-only); UX-4 surfaces are subject-shape-agnostic — unit-covered at per-bucket test files",
    },
];
...
test("@webkit @touch UX-4-Z cluster — case-fix + home + sidebar collapse + close-fallback + *serv route + members sort + scroll-on-activate + settings + upload-TTL persistence + admin window (parity matrix)", async ({ page, context, }) => {
    for (const klass of CLASSES) {
        ...
        await expect(homePane.locator(".home-pane-networks-section .home-pane-title")).toContainText("Networks");
        ...
        const networkHeader = page.locator(`.bottom-bar-network-header[data-network-slug="${NETWORK_SLUG}"]`);
        ...
        const selectedTabText = (await selectedTab.count()) === 0 ? null : await selectedTab.first().textContent();
        expect(redirectedToHome > 0 || (selectedTabText !== null && !selectedTabText.includes(CHANNEL))).toBe(true);
        ...
        if (ttlVisible) {
            ...
            test.info().annotations.push({
                type: "skip-bucket-M",
                description: "upload-ttl-select not rendered (active host has no ttlOptions in e2e env) — arm skipped",
            });
        }
