...
test("uploads-3 #118 — multi-file picker uploads ALL files sequentially → two 📸 links", async ({ page, }) => {
    const vjt = specUser();
    ...
    const slugs = new Set(texts.flatMap((t) => Array.from(t.matchAll(/\/uploads\/([a-z2-7]{26})/g)).map((m) => m[1])));
