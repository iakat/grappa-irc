...
test("#231 — /lusers surfaces LusersCard in the CURRENT (channel) window, not $server", async ({ page, }) => {
    const vjt = specUser();
    ...
    const dt = (label) => card.locator("dt", { hasText: new RegExp(`^${label}$`) });
    const ddFor = (label) => dt(label).locator("xpath=following-sibling::dd[1]");
