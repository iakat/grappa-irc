...
import { bootVisitorContext, composeSend, expectShellReady, selectChannel, waitForUserTopicReady, } from "../fixtures/cicchettoPage";
...
async function waitForRecoverable(token, slug, expected, timeoutMs = 45_000) {
    ...
    throw new Error(`waitForRecoverable(${slug}): never reached recoverable=${expected} (last=${last})`);
}
test.describe("#581 recover-identity (real services)", () => {
    ...
    test("first-login (anon credential) → recoverable is false and the 🔑 button is absent", async ({ browser, }) => {
        ...
        try {
            ...
            await expect(page.getByTestId(`home-recover-identity-${visitor.network_slug}`)).toHaveCount(0);
        }
        finally {
            if (ctx)
                await ctx.close();
    ...
    test("a returning visitor on a Guest nick recovers a held registered nick via RECOVER → +r", async ({ browser, }) => {
        ...
        try {
            ...
            await expect(modal.locator('[data-testid="recover-modal-step"][data-step="recover"]')).toBeVisible({ timeout: 20_000 });
