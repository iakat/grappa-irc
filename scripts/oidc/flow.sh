...
$CURL -o /dev/null -X POST -d "username=$KUSER" "$URL/ui/login/begin"
...
$CURL -o /dev/null -X POST -d "password=$KPASS" "$URL/ui/login/pw"
...
CB=$(grep -i '^location' "$TMP/resume" | tr -d '\r' | cut -d' ' -f2)
if [ -z "$CB" ]; then  # consent page: submit its token, then take the new code
    CT=$(grep -oE 'name="consent_token"[^>]*value="[^"]*"' "$TMP/resume-body" | grep -oE 'value="[^"]*"' | cut -d'"' -f2)
    $CURL -o /dev/null -D "$TMP/consent-post" -X POST -d "consent_token=$CT" "$URL/ui/oauth2/consent"
    CB=$(grep -i '^location:' "$TMP/consent-post" | tr -d '\r' | cut -d' ' -f2)
fi
...
FRAG=$(grep -i '^location:' "$TMP/callback" | tr -d '\r' | awk '{print $2}' | sed 's|^/login#oidc=||')
